# 1 "./dht/dhtcore/test/NodeStore_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./dht/dhtcore/test/NodeStore_test.c"
# 15 "./dht/dhtcore/test/NodeStore_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x08713912636347592;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x027026211614856166;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x07068660682133265;
...
struct Linker_x06027609902463793;
...
struct Linker_x06260205820179277;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x005381250071688326;
...
struct Linker_x04258830523664747;
...
struct Linker_x09742188903644835;
...
struct Linker_x02199026781251059;
...
# 17 "./dht/dhtcore/test/NodeStore_test.c" 2

# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x05093256563084225;
# 34 "./dht/Address.h"
struct Address
{
...
# 19 "./dht/dhtcore/test/NodeStore_test.c" 2
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x06636924276747787;
...
struct Linker_x05255974659832445;
...
struct Linker_x01188257632989258;
...
# 20 "./dht/dhtcore/test/NodeStore_test.c" 2
...
struct Linker_x03079805779034379;
...
struct Linker_x008645300625571628;
...
# 238 "./dht/dhtcore/NodeStore.h"
                                                              fullVerify);
# 22 "./dht/dhtcore/test/NodeStore_test.c" 2
# 1 "./switch/NumberCompress.h" 1
# 23 "./switch/NumberCompress.h"
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x019582264499638202;
...
# 23 "./dht/dhtcore/test/NodeStore_test.c" 2
...
# 25 "./dht/dhtcore/test/NodeStore_test.c" 2
# 1 "./util/log/FileWriterLog.h" 1
# 21 "./util/log/FileWriterLog.h"
struct Linker_x0027294715062866404;
...
# 26 "./dht/dhtcore/test/NodeStore_test.c" 2
...
# 43 "./dht/dhtcore/test/NodeStore_test.c" 3 4
   ((void *)0)

# 44 "./dht/dhtcore/test/NodeStore_test.c"
};
...
static void addNode(struct NodeStore* ns, uint8_t* address, struct Allocator* alloc)
{
    ...
    do { if (!((NodeStore_discoverNode(ns, addr, scheme, 0, 100)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (50), "(NodeStore_discoverNode(ns, addr, scheme, 0, 100))"); } } while (0);
}
                      ...
                      struct Allocator* alloc)
{
    ...
    for (int i = (int)list->size - 1; i >= 0; i--) {
        ...
        do { if (logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(logger, Log_Level_INFO, "NodeStore_test.c", 61, "output: [%s]", str->bytes); } } while (0);
        do { if (!((expectedOutputs[j]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (62), "(expectedOutputs[j])"); } } while (0);
        do { if (!((expectedOutputs[j] == list->nodes[i]->address.path))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (63), "(expectedOutputs[j] == list->nodes[i]->address.path)"); } } while (0);
        j++;
    }
    do { if (!((!expectedOutputs[j]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (66), "(!expectedOutputs[j])"); } } while (0);
}
...
static void genAddress(uint8_t* addr, struct Random* rand)
{
    ...
    do { if (!((publicKeyBase32))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (76), "(publicKeyBase32)"); } } while (0);
                         ...
                         struct Random* rand)
{
    ...
    do { if (!((myAddr))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_test.c"), (94), "(myAddr)"); } } while (0);
    struct NodeStore* ns = NodeStore_new(myAddr, alloc, base, logger, 
# 95 "./dht/dhtcore/test/NodeStore_test.c" 3 4
                                                                     ((void *)0)
# 95 "./dht/dhtcore/test/NodeStore_test.c"
                                                                         );
    NodeStore_setFullVerify(ns, 
# 96 "./dht/dhtcore/test/NodeStore_test.c" 3 4
                               1
# 96 "./dht/dhtcore/test/NodeStore_test.c"
                                   );
...
int NodeStore_test_main(int argc, char** argv);int NodeStore_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((1<<20),"NodeStore_test.c",112);
    struct Log* logger = FileWriterLog_new(
# 113 "./dht/dhtcore/test/NodeStore_test.c" 3
                                          (__acrt_iob_func(1))
# 113 "./dht/dhtcore/test/NodeStore_test.c"
                                                , alloc);
    struct EventBase* base = EventBase_new(alloc);
    struct Random* rand = Random_new(alloc, 
# 115 "./dht/dhtcore/test/NodeStore_test.c" 3 4
                                           ((void *)0)
# 115 "./dht/dhtcore/test/NodeStore_test.c"
                                               , 
# 115 "./dht/dhtcore/test/NodeStore_test.c" 3 4
                                                 ((void *)0)
# 115 "./dht/dhtcore/test/NodeStore_test.c"
                                                     );
# 130 "./dht/dhtcore/test/NodeStore_test.c"
    getPeersTest(ADDRS, base, logger, alloc, rand);

    Allocator__free((alloc),"NodeStore_test.c",132);
    return 0;
}
