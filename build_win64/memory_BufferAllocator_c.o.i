# 1 "memory/BufferAllocator.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "memory/BufferAllocator.c"
# 15 "memory/BufferAllocator.c"
...
struct Linker_x05848690853199903;
...
# 18 "memory/BufferAllocator.c" 2
# 1 "./memory/BufferAllocator.h" 1
# 18 "./memory/BufferAllocator.h"
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x05657624659028453;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x05310600525434179;
...
__attribute__ ((alloc_size(3)))
typedef void* (* Allocator_Provider)(struct BufferAllocator_pvt* ctx,
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                struct BufferAllocator_pvt* providerContext,
                                const char* fileName,
                                int lineNum);
# 19 "./memory/BufferAllocator.h" 2
...
struct Linker_x06491416048337975;
# 34 "./memory/BufferAllocator.h"
struct Allocator* BufferAllocator__new(void* buffer,
                                       unsigned long length,
                                       char* file,
                                       int line);
# 19 "memory/BufferAllocator.c" 2
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0888831649545617;
...
# 20 "memory/BufferAllocator.c" 2
# 33 "memory/BufferAllocator.c"
struct BufferAllocator_pvt
{
...
# 61 "memory/BufferAllocator.c"
static void* allocatorMalloc(struct BufferAllocator_pvt* context, unsigned long length)
{
    (__extension__ ({ __typeof__(context) Identity_ptr = (context); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7f367ed576e17cbcull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("BufferAllocator.c"), (63), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7f367ed576e17cbcull))"); } } while (0); Identity_ptr; }));
    char* pointer = ((char*) ((uintptr_t)( ((char*)(context->pointer)) + (16) - 1) & ~ ((16) - 1)));
    char* endOfAlloc = pointer + length;

    if (endOfAlloc >= context->endPointer) {
        Except__throw("BufferAllocator.c", 68, context->onOOM, "BufferAllocator ran out of memory.");
    }

    if (endOfAlloc < context->pointer) {
        Except__throw("BufferAllocator.c", 72, context->onOOM, "BufferAllocator integer overflow.");
    }
...
{
    if (original == 
# 84 "memory/BufferAllocator.c" 3 4
                   ((void *)0)
# 84 "memory/BufferAllocator.c"
                       ) {
    ...
    if (size == 0) {
        return 
# 95 "memory/BufferAllocator.c" 3 4
              ((void *)0)
# 95 "memory/BufferAllocator.c"
                  ;
    }

    void* newAlloc = allocatorMalloc(context, size);
    if (newAlloc != original) {
        do { if (!(((char*)newAlloc > (char*)original + original->size))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("BufferAllocator.c"), (100), "((char*)newAlloc > (char*)original + original->size)"); } } while (0);
        Bits__memcpy(newAlloc,original,original->size,"BufferAllocator.c",101);
    }
    return newAlloc;
}
                                       ...
                                       int line)
{
    ...
    (&stackAlloc)->Identity_verifier = ((unsigned long) 0x7f367ed576e17cbcull);
    struct BufferAllocator_pvt* alloc =
        allocatorMalloc(&stackAlloc, sizeof(struct BufferAllocator_pvt));
    Bits__memcpy(alloc,&stackAlloc,sizeof(struct BufferAllocator_pvt),"BufferAllocator.c",120);
