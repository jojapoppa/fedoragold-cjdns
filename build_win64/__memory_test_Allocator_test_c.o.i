# 1 "./memory/test/Allocator_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./memory/test/Allocator_test.c"
# 15 "./memory/test/Allocator_test.c"
# 1 "./util/Assert.h" 1
# 18 "./util/Assert.h"
# 1 "./util/Gcc.h" 1
# 18 "./util/Gcc.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07358641556995789;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
void Assert_failure(const char* format, ...);
# 16 "./memory/test/Allocator_test.c" 2
...
# 17 "./memory/test/Allocator_test.c" 2
...
# 18 "./memory/test/Allocator_test.c" 2
...
struct Linker_x036308823897087583;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 20 "./memory/test/Allocator_test.c" 2
...
{
    struct Allocator* alloc = MallocAllocator__new((2048),"Allocator_test.c",42);
    int calls = 0;
    struct TestStruct* ts = Allocator__clone((alloc),((&(struct TestStruct) { .value = increment(&calls) })),sizeof(*((&(struct TestStruct) { .value = increment(&calls) }))),"Allocator_test.c",44)

       ;
    do { if (!((calls == 1))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (47), "(calls == 1)"); } } while (0);
    do { if (!((ts->value == 1))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (48), "(ts->value == 1)"); } } while (0);
    Allocator__free((alloc),"Allocator_test.c",49);
}

static void structureSizes()
{
    struct Allocator* alloc = MallocAllocator__new((2048),"Allocator_test.c",54);
    size_t bytesUsed;

    bytesUsed = Allocator_bytesAllocated(alloc);
    do { if (!((bytesUsed == sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_FirstCtx)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (58), "(bytesUsed == sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_FirstCtx))"); } } while (0);
    Allocator__malloc((alloc),(25),"Allocator_test.c",59);
    bytesUsed += (((25 / sizeof(char*)) + 1) * sizeof(char*)) + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (61), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    struct Allocator* child = Allocator__child((alloc),"Allocator_test.c",63);
    bytesUsed += sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_pvt);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (65), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__malloc((child),(30),"Allocator_test.c",67);
    bytesUsed += 32 + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (69), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__free((child),"Allocator_test.c",71);
    bytesUsed -= 32 + sizeof(struct Allocator_Allocation_pvt) + sizeof(long);
    bytesUsed -= sizeof(struct Allocator_Allocation_pvt) + sizeof(long) + sizeof(struct Allocator_pvt);
    do { if (!((Allocator_bytesAllocated(alloc) == bytesUsed))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Allocator_test.c"), (74), "(Allocator_bytesAllocated(alloc) == bytesUsed)"); } } while (0);

    Allocator__free((alloc),"Allocator_test.c",76);
}

int Allocator_test_main(int argc, char** argv);int Allocator_test_main(int argc, char** argv)
{
    allocatorClone();
    structureSizes();
    return 0;
