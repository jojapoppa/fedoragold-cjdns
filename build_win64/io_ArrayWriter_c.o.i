...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x08500257558121265;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x013795013425869307;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 20 "./io/ArrayWriter.h" 2

struct Linker_x009629448720830003;
# 30 "./io/ArrayWriter.h"
struct Writer* ArrayWriter_new(void* writeTo,
                               unsigned long length,
                               struct Allocator* allocator);
# 16 "io/ArrayWriter.c" 2
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x010294281121477522;
...
{
    struct ArrayWriter_context* context = (__extension__ ({ __typeof__((struct ArrayWriter_context*) writer) Identity_ptr = ((struct ArrayWriter_context*) writer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x619fe70b5d9ab9beull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ArrayWriter.c"), (31), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x619fe70b5d9ab9beull))"); } } while (0); Identity_ptr; }));
    ...
    Bits__memcpy(context->pointer,toWrite,length,"ArrayWriter.c",46);
...
struct Writer* ArrayWriter_new(void* writeToBuffer,
                               ...
                               struct Allocator* alloc)
{
    struct ArrayWriter_context* context = Allocator__clone((alloc),((&(struct ArrayWriter_context) { .generic = { .write = write }, .beginPointer = writeToBuffer, .pointer = writeToBuffer, .endPointer = (char*) writeToBuffer + length })),sizeof(*((&(struct ArrayWriter_context) { .generic = { .write = write }, .beginPointer = writeToBuffer, .pointer = writeToBuffer, .endPointer = (char*) writeToBuffer + length }))),"ArrayWriter.c",58)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0x619fe70b5d9ab9beull);
