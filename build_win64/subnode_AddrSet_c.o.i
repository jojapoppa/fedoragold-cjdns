...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x017395608776305638;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x019594497583349724;
...
# 17 "subnode/AddrSet.c" 2
# 1 "./subnode/AddrSet.h" 1
# 18 "./subnode/AddrSet.h"
...
struct Linker_x010462690083999471;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x004217209080115336;
...
struct Linker_x038966654867413797;
...
struct Linker_x09672488695572887;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x07383010558904126;
...
# 21 "./util/ArrayList.h"
struct Linker_x011034132722228174;
...
# 60 "./util/ArrayList.h"
struct ArrayList_OfAddrs {
    int length;
};

static inline struct ArrayList_OfAddrs* ArrayList_OfAddrs_new(struct Allocator* alloc)
{
    return (struct ArrayList_OfAddrs*) ArrayList_new(alloc, 8);
}

static inline struct Elem* ArrayList_OfAddrs_get(struct ArrayList_OfAddrs* list, int number)
{
    return (struct Elem*) ArrayList_get((struct ArrayList*) list, number);
}

static inline int ArrayList_OfAddrs_put(struct ArrayList_OfAddrs* list,
                                          int number,
                                          struct Elem* val)
{
...
static inline int ArrayList_OfAddrs_add(struct ArrayList_OfAddrs* list, void* val)
{
    return ArrayList_put((struct ArrayList*) list, list->length, val);
}

static inline struct Elem* ArrayList_OfAddrs_shift(struct ArrayList_OfAddrs* list)
{
    return (struct Elem*) ArrayList_remove((struct ArrayList*) list, 0);
}

static inline struct Elem* ArrayList_OfAddrs_pop(struct ArrayList_OfAddrs* list)
{
    return (struct Elem*) ArrayList_remove((struct ArrayList*) list, list->length - 1);
}

static inline struct Elem* ArrayList_OfAddrs_remove(struct ArrayList_OfAddrs* list, int num)
{
    return (struct Elem*) ArrayList_remove((struct ArrayList*) list, num);
}


static inline int ArrayList_OfAddrs_sort_compare(const void* a, const void* b)
{
    return comparePeerAddresses(((struct Elem**)a)[0], ((struct Elem**)b)[0]);
}
static inline void ArrayList_OfAddrs_sort(struct ArrayList_OfAddrs* list)
{
    ArrayList_sort((struct ArrayList*) list, ArrayList_OfAddrs_sort_compare);
}



static inline void* ArrayList_OfAddrs_clone(struct ArrayList_OfAddrs* l, struct Allocator* alloc)
{
    return ArrayList_clone((struct ArrayList*) l, alloc);
}
# 36 "subnode/AddrSet.c" 2
...
int AddrSet_indexOf(struct AddrSet* as, struct Address* addr)
{
    struct AddrSet_pvt* ap = (__extension__ ({ __typeof__((struct AddrSet_pvt*) as) Identity_ptr = ((struct AddrSet_pvt*) as); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrSet.c"), (57), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull))"); } } while (0); Identity_ptr; }));
    return indexOf(ap, addr);
}

void AddrSet_add(struct AddrSet* as, struct Address* addr)
{
    struct AddrSet_pvt* ap = (__extension__ ({ __typeof__((struct AddrSet_pvt*) as) Identity_ptr = ((struct AddrSet_pvt*) as); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrSet.c"), (63), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull))"); } } while (0); Identity_ptr; }));
    int idx = indexOf(ap, addr);
    if (idx != -1) { return; }
    struct Allocator* alloc = Allocator__child((ap->alloc),"AddrSet.c",66);
    struct Elem* el = Allocator__calloc((alloc),(sizeof(struct Elem)),(1),"AddrSet.c",67);
    el->alloc = alloc;
    Bits__memcpy(&el->addr,addr,sizeof(struct Address),"AddrSet.c",69);
    (el)->Identity_verifier = ((unsigned long) 0x6d1d8038bb1e765dull);
...
void AddrSet_remove(struct AddrSet* as, struct Address* addr)
{
    struct AddrSet_pvt* ap = (__extension__ ({ __typeof__((struct AddrSet_pvt*) as) Identity_ptr = ((struct AddrSet_pvt*) as); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrSet.c"), (78), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull))"); } } while (0); Identity_ptr; }));
    int idx = indexOf(ap, addr);
    if (idx == -1) { return; }
    struct Elem* el = ArrayList_OfAddrs_get(ap->addrList, idx);
    ArrayList_OfAddrs_remove(ap->addrList, idx);
    Allocator__free((el->alloc),"AddrSet.c",83);
    ap->pub.length = ap->addrList->length;
}

void AddrSet_flush(struct AddrSet* as)
{
    struct AddrSet_pvt* ap = (__extension__ ({ __typeof__((struct AddrSet_pvt*) as) Identity_ptr = ((struct AddrSet_pvt*) as); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrSet.c"), (89), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull))"); } } while (0); Identity_ptr; }));
    for (int i = 0; i < ap->addrList->length; i++) {
        struct Elem* el = ArrayList_OfAddrs_get(ap->addrList, i);
        ArrayList_OfAddrs_remove(ap->addrList, i);
        Allocator__free((el->alloc),"AddrSet.c",93);
    }
    ap->pub.length = 0;
}
...
struct Address* AddrSet_get(struct AddrSet* as, int i)
{
    struct AddrSet_pvt* ap = (__extension__ ({ __typeof__((struct AddrSet_pvt*) as) Identity_ptr = ((struct AddrSet_pvt*) as); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AddrSet.c"), (100), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x6d1d8038bb1e765dull))"); } } while (0); Identity_ptr; }));
    struct Elem* el = ArrayList_OfAddrs_get(ap->addrList, i);
    if (el) { return &el->addr; }
    return 
# 103 "subnode/AddrSet.c" 3 4
          ((void *)0)
# 103 "subnode/AddrSet.c"
              ;
}

struct AddrSet* AddrSet_new(struct Allocator* allocator)
{
    struct Allocator* alloc = Allocator__child((allocator),"AddrSet.c",108);
    struct AddrSet_pvt* out = Allocator__calloc((alloc),(sizeof(struct AddrSet_pvt)),(1),"AddrSet.c",109);
    out->alloc = alloc;
    out->addrList = ArrayList_OfAddrs_new(alloc);
    (out)->Identity_verifier = ((unsigned long) 0x6d1d8038bb1e765dull);
    return &out->pub;
}
