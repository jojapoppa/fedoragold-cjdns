# 1 "util/log/IndirectLog.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "util/log/IndirectLog.c"
# 16 "util/log/IndirectLog.c"
# 1 "./util/log/Log_impl.h" 1
# 18 "./util/log/Log_impl.h"
# 1 "./util/log/Log.h" 1
# 18 "./util/log/Log.h"
# 1 "./util/Gcc.h" 1
# 18 "./util/Gcc.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/log/Log.h" 2
struct Linker_x08419565967264011;
...
# 17 "util/log/IndirectLog.c" 2
# 1 "./util/log/IndirectLog.h" 1
# 18 "./util/log/IndirectLog.h"
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x0372302782102337;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06379781506209987;
...
# 21 "util/log/IndirectLog.c" 2


# 22 "util/log/IndirectLog.c"
...
static void doLog(struct Log* genericLog,
                  ...
                  va_list args)
{
    struct IndirectLog_pvt* il = (__extension__ ({ __typeof__((struct IndirectLog_pvt*) genericLog) Identity_ptr = ((struct IndirectLog_pvt*) genericLog); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x79a7d583b5346cd6ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("IndirectLog.c"), (36), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x79a7d583b5346cd6ull))"); } } while (0); Identity_ptr; }));
    if (il && il->wrapped) {
...
struct Log* IndirectLog_new(struct Allocator* alloc)
{
    struct IndirectLog_pvt* il = Allocator__clone((alloc),((&(struct IndirectLog_pvt) { .log = { .print = doLog } })),sizeof(*((&(struct IndirectLog_pvt) { .log = { .print = doLog } }))),"IndirectLog.c",44)



       ;
    (il)->Identity_verifier = ((unsigned long) 0x79a7d583b5346cd6ull);
    return &il->log;
}

void IndirectLog_set(struct Log* indirectLog, struct Log* wrapped)
{
    struct IndirectLog_pvt* il = (__extension__ ({ __typeof__((struct IndirectLog_pvt*) indirectLog) Identity_ptr = ((struct IndirectLog_pvt*) indirectLog); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x79a7d583b5346cd6ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("IndirectLog.c"), (55), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x79a7d583b5346cd6ull))"); } } while (0); Identity_ptr; }));
    il->wrapped = wrapped;
}
