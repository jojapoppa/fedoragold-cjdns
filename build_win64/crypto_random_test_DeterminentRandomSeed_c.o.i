# 1 "crypto/random/test/DeterminentRandomSeed.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "crypto/random/test/DeterminentRandomSeed.c"
# 15 "crypto/random/test/DeterminentRandomSeed.c"
# 1 "./crypto/random/test/DeterminentRandomSeed.h" 1
# 18 "./crypto/random/test/DeterminentRandomSeed.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x033586965762834997;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x04579910866090158;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x02526504655295698;
...
struct Linker_x040747116874254563;
...
# 19 "./crypto/random/test/DeterminentRandomSeed.h" 2


struct Linker_x08984953759409591;

struct RandomSeed* DeterminentRandomSeed_new(struct Allocator* alloc, uint8_t buff[64]);
# 16 "crypto/random/test/DeterminentRandomSeed.c" 2
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x09404851863613166;
...
# 17 "crypto/random/test/DeterminentRandomSeed.c" 2
...
static int get(struct RandomSeed* rand, uint64_t buff[8])
{
    ...
    struct DeterminentRandomSeed* drs = (__extension__ ({ __typeof__((struct DeterminentRandomSeed*) rand) Identity_ptr = ((struct DeterminentRandomSeed*) rand); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xb8ca58aea264ef74ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("DeterminentRandomSeed.c"), (32), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xb8ca58aea264ef74ull))"); } } while (0); Identity_ptr; }));
    Bits__memcpy(buff,drs->buff,64,"DeterminentRandomSeed.c",33);
    return 0;
}

struct RandomSeed* DeterminentRandomSeed_new(struct Allocator* alloc, uint8_t buff[64])
{
    struct DeterminentRandomSeed* drs = Allocator__clone((alloc),((&(struct DeterminentRandomSeed) { .rs = { .get = get, .name = "DeterminentRandomSeed TESTING ONLY, THIS SHOULD NEVER BE USED IN PRODUCTION!" } })),sizeof(*((&(struct DeterminentRandomSeed) { .rs = { .get = get, .name = "DeterminentRandomSeed TESTING ONLY, THIS SHOULD NEVER BE USED IN PRODUCTION!" } }))),"DeterminentRandomSeed.c",39)




       ;
    (drs)->Identity_verifier = ((unsigned long) 0xb8ca58aea264ef74ull);
    if (buff) {
        Bits__memcpy(drs->buff,buff,64,"DeterminentRandomSeed.c",47);
    } else {
        __builtin_memset(drs->buff,4,64);
    }
    return &drs->rs;
}
