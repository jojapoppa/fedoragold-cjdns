...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x03680004119632565;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05950792079152578;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x08446847647567224;
...
struct Linker_x0500860839084736;
...
struct Linker_x0460708282698715;
# 34 "./dht/Address.h"
struct Address
{
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x027658589857800253;
...
struct Linker_x0318464696380502;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x09235639135863138;
...
struct Linker_x0014238986627431727;
...
struct Linker_x037972244165740854;
...
struct Linker_x08707492927295621;
...
# 19 "./util/AddrTools.h"
struct Linker_x0018157893723912633;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x03805894515784687;
...
struct Linker_x02441373926518804;
...
# 48 "net/ControlHandler.c"
    ...
    Message_push(msg, &rh->sh, 12, 
# 51 "net/ControlHandler.c" 3 4
                                                 ((void *)0)
# 51 "net/ControlHandler.c"
                                                     );
    Message_push32(msg, 0xffffffff, 
# 52 "net/ControlHandler.c" 3 4
                                   ((void *)0)
# 52 "net/ControlHandler.c"
                                       );
    Message_push32(msg, PFChan_Core_SWITCH_ERR, 
# 53 "net/ControlHandler.c" 3 4
                                               ((void *)0)
# 53 "net/ControlHandler.c"
                                                   );
...
# 69 "net/ControlHandler.c"
    ...
    Message_shift(msg, -4, 
# 73 "net/ControlHandler.c" 3 4
                                            ((void *)0)
# 73 "net/ControlHandler.c"
                                                );
...
# 109 "net/ControlHandler.c"
    ...
    Message_shift(msg, 4, 
# 120 "net/ControlHandler.c" 3 4
                                           ((void *)0)
# 120 "net/ControlHandler.c"
                                               );
...
# 125 "net/ControlHandler.c" 3 4
                                        ((void *)0)
# 125 "net/ControlHandler.c"
                                            );
...
# 162 "net/ControlHandler.c"
    ...
    Message_shift(msg, (56 + 12), 
# 185 "net/ControlHandler.c" 3 4
                                        ((void *)0)
# 185 "net/ControlHandler.c"
                                            );
    struct RouteHeader* routeHeader = (struct RouteHeader*) msg->bytes;
    __builtin_memset(routeHeader,0,(56 + 12));
    SwitchHeader_setVersion(&routeHeader->sh, 1);
...
{
    struct ControlHandler_pvt* ch = (__extension__ ({ __typeof__((struct ControlHandler_pvt*) coreIf) Identity_ptr = ((struct ControlHandler_pvt*) coreIf); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ControlHandler.c"), (206), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull))"); } } while (0); Identity_ptr; }));

    struct RouteHeader routeHdr;
    Message_pop(msg, &routeHdr, (56 + 12), 
# 209 "net/ControlHandler.c" 3 4
                                                 ((void *)0)
# 209 "net/ControlHandler.c"
                                                     );
    uint8_t labelStr[20];
...
# 225 "net/ControlHandler.c"
            ...
            || ctrl->header.type_be == ((uint16_t)(__extension__ ({ uint16_t _x = (((uint16_t)(4))); _x >> 8 | _x << 8; }))))
    {
        ...
        Message_push(msg, &routeHdr, (56 + 12), 
# 242 "net/ControlHandler.c" 3 4
                                                      ((void *)0)
# 242 "net/ControlHandler.c"
                                                          );
        ...
        Message_push(msg, &routeHdr, (56 + 12), 
# 250 "net/ControlHandler.c" 3 4
                                                      ((void *)0)
# 250 "net/ControlHandler.c"
                                                          );
        Message_push32(msg, 0xffffffff, 
# 251 "net/ControlHandler.c" 3 4
                                       ((void *)0)
# 251 "net/ControlHandler.c"
                                           );
        Message_push32(msg, PFChan_Core_CTRL_MSG, 
# 252 "net/ControlHandler.c" 3 4
                                                 ((void *)0)
# 252 "net/ControlHandler.c"
                                                     );
        return Iface_next(&ch->eventIf, msg);
    }
...
# 266 "net/ControlHandler.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ControlHandler.c"), (266), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull))"); } } while (0); Identity_ptr; })); }));
    return Iface_next(&ch->pub.coreIf, msg);
}
...
# 273 "net/ControlHandler.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ControlHandler.c"), (273), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3a9d9c4f3af98295ull))"); } } while (0); Identity_ptr; })); }));
    enum PFChan_Pathfinder ev = Message_pop32(msg, 
# 274 "net/ControlHandler.c" 3 4
                                                  ((void *)0)
# 274 "net/ControlHandler.c"
                                                      );
    ...
    Message_pop32(msg, 
# 276 "net/ControlHandler.c" 3 4
                      ((void *)0)
# 276 "net/ControlHandler.c"
                          );

    struct PFChan_Node node;
    Message_pop(msg, &node, 64, 
# 279 "net/ControlHandler.c" 3 4
                                             ((void *)0)
# 279 "net/ControlHandler.c"
                                                 );
                                          ...
                                          uint8_t myPublicKey[32])
{
    ...
    (ch)->Identity_verifier = ((unsigned long) 0x3a9d9c4f3af98295ull);
    return &ch->pub;
}
