...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x04492526578403362;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09572994964520805;
...
struct Linker_x035575093392648105;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06841464891419506;
...
struct Linker_x0382448562679953;
...
struct Linker_x020569628301253173;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x05033264047384085;
...
struct Linker_x07144888164789993;
...
# 21 "./util/ArrayList.h"
struct Linker_x07892902558476698;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x04128388099882443;
...
struct Linker_x08337070602867254;
...
struct Linker_x0843240434762152;
...
struct Linker_x01348691161620139;
...
struct Linker_x0882540067220362;
...
# 1 "./benc/Dict.h" 1
# 21 "./benc/Dict.h"
struct Linker_x008065669726957259;
...
struct Linker_x0015661874631315298;
...
struct Linker_x038697285512710056;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x0981566647730115;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x0585181472416815;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x09245036497698911;
...
struct Linker_x004608317694638675;
...
struct Linker_x0025939708560091868;
...
struct Linker_x0178444126156867;
...
struct Linker_x031651838665966503;
...
struct Linker_x05302382865562985;
...
struct Linker_x05205182628755145;
...
struct Linker_x05651052485886623;
...
struct Linker_x05600023868840098;
...
struct Linker_x08017001567671582;
...
struct Linker_x09370158262845736;
...
struct Linker_x0989606965246357;
...
struct Linker_x07899684041048827;
...
struct Linker_x037172538871351546;
...
struct Linker_x048766721175194716;
...
struct Linker_x08307886465066519;
...
struct Linker_x08486154710568818;
...
# 22 "./net/TUNAdapter.h"
struct Linker_x08634044998218722;
...
struct Linker_x023167816246932138;
...
struct Linker_x01382136081006904;
...
struct Linker_x03649835355133517;
...
{
    do { if (!((!((uintptr_t)msg->bytes % 4) || !"alignment fault"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (65), "(!((uintptr_t)msg->bytes % 4) || !\"alignment fault\")"); } } while (0);
    do { if (!((!(msg->capacity % 4) || !"length fault"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (66), "(!(msg->capacity % 4) || !\"length fault\")"); } } while (0);
    do { if (!((((int)msg->capacity >= msg->length) || !"length fault0"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (67), "(((int)msg->capacity >= msg->length) || !\"length fault0\")"); } } while (0);

    do { if (srcTf->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(srcTf->logger, Log_Level_DEBUG, "TestFramework.c", 69, "Transferring message to [%p] - message length [%d]\n", (void*)dest, msg->length); } } while (0)
                                       ;
    ...
    if (msg->alloc) {
        ...
        Allocator__adopt((srcTf->alloc),(msg->alloc),"TestFramework.c",78);
    }
...
# 91 "test/TestFramework.c" 3 4
       __builtin_offsetof (
# 91 "test/TestFramework.c"
       struct TestFramework_Link
# 91 "test/TestFramework.c" 3 4
       , 
# 91 "test/TestFramework.c"
...
# 91 "test/TestFramework.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe6f42baece44b505ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (91), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe6f42baece44b505ull))"); } } while (0); Identity_ptr; })); }));
...
# 98 "test/TestFramework.c" 3 4
       __builtin_offsetof (
# 98 "test/TestFramework.c"
       struct TestFramework_Link
# 98 "test/TestFramework.c" 3 4
       , 
# 98 "test/TestFramework.c"
...
# 98 "test/TestFramework.c"
       )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xe6f42baece44b505ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (98), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xe6f42baece44b505ull))"); } } while (0); Identity_ptr; })); }));
...
# 109 "test/TestFramework.c" 3
                                                 (__acrt_iob_func(1))
# 109 "test/TestFramework.c"
                                                       , allocator);
...
# 114 "test/TestFramework.c" 3 4
                                            ((void *)0)
# 114 "test/TestFramework.c"
                                                );
    }
...
# 144 "test/TestFramework.c" 3 4
                                                                                  ((void *)0)
# 144 "test/TestFramework.c"
                                                                                      );
        pf->fullVerify = 
# 145 "test/TestFramework.c" 3 4
                        1
# 145 "test/TestFramework.c"
                            ;
    ...
    struct TestFramework* tf = Allocator__calloc((allocator),(sizeof(struct TestFramework)),(1),"TestFramework.c",153);
    (tf)->Identity_verifier = ((unsigned long) 0xe6f42baece44b505ull);
    tf->alloc = allocator;
    tf->rand = rand;
    tf->eventBase = base;
    tf->logger = logger;
...
void TestFramework_assertLastMessageUnaltered(struct TestFramework* tf)
{
    ...
    do { if (!((a->length == b->length))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (179), "(a->length == b->length)"); } } while (0);
    do { if (!((a->padding == b->padding))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (180), "(a->padding == b->padding)"); } } while (0);
    do { if (!((!__builtin_memcmp(a->bytes,b->bytes,a->length)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("TestFramework.c"), (181), "(!__builtin_memcmp(a->bytes,b->bytes,a->length))"); } } while (0);
}

void TestFramework_linkNodes(struct TestFramework* client,
                             struct TestFramework* server,
                             
# 186 "test/TestFramework.c" 3 4
                            _Bool 
# 186 "test/TestFramework.c"
                                 beacon)
{

    struct TestFramework_Link* link =
        Allocator__calloc((client->alloc),(sizeof(struct TestFramework_Link)),(1),"TestFramework.c",190);
    (link)->Identity_verifier = ((unsigned long) 0xe6f42baece44b505ull);
...
# 227 "test/TestFramework.c" 3 4
                                  ((void *)0)
# 227 "test/TestFramework.c"
                                      ,
                                   
# 228 "test/TestFramework.c" 3 4
                                  ((void *)0)
# 228 "test/TestFramework.c"
                                      ,
                                   client->alloc);
    }
}
...
# 235 "test/TestFramework.c" 3 4
                                              ((void *)0)
# 235 "test/TestFramework.c"
                                                  );
    ...
    ip->nextHeader = 123;
    ip->hopLimit = 255;
    Bits__memcpy(ip->sourceAddr,srcAddr,16,"TestFramework.c",243);
    Bits__memcpy(ip->destinationAddr,destAddr,16,"TestFramework.c",244);
    (((uint8_t*) ip)[0] |= ( (sizeof(*ip) == 20) ? 4 : 6 ) << 4);
}

int TestFramework_peerCount(struct TestFramework* node)
{
    struct Allocator* alloc = Allocator__child((node->alloc),"TestFramework.c",250);
    struct InterfaceController_PeerStats* statsOut = 
# 251 "test/TestFramework.c" 3 4
                                                    ((void *)0)
# 251 "test/TestFramework.c"
                                                        ;
    ...
    Allocator__free((alloc),"TestFramework.c",257);
    return out;
}

int TestFramework_sessionCount(struct TestFramework* node)
{
    struct Allocator* alloc = Allocator__child((node->alloc),"TestFramework.c",263);
    ...
    Allocator__free((alloc),"TestFramework.c",271);
    return count;
}
