...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x03367719906254454;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x03654485616831784;
...
struct Linker_x011800792014273176;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x07729371760102282;
...
struct Linker_x006006754595720576;
...
struct Linker_x008886600924300114;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x06748889861737992;
...
struct Linker_x0883678355411021;
...
# 1 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/ge.h" 1
# 18 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/ge.h"
# 1 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/fe.h" 1



# 1 "build_win64/dependencies/cnacl/jsbuild/include_internal/crypto_int32.h" 1
# 1 "build_win64/dependencies/cnacl/jsbuild/include_internal/crypto_types.h" 1
...
# 1 "build_win64/dependencies/cnacl/jsbuild/include_internal/crypto_int32.h" 2
# 5 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/fe.h" 2

typedef crypto_int32 fe[10];
# 35 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/fe.h"
...
# 19 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/ge.h" 2
...
# 74 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/ge.h"
...
# 19 "crypto/Sign.c" 2
# 1 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/sc.h" 1
# 12 "./node_build/dependencies/cnacl/crypto_sign/ed25519/ref10/sc.h"
...
# 21 "crypto/Sign.c" 2
# 1 "build_win64/dependencies/cnacl/jsbuild/include/crypto_sign_ed25519.h" 1
# 15 "build_win64/dependencies/cnacl/jsbuild/include/crypto_sign_ed25519.h"
...
void Sign_signingKeyPairFromCurve25519(uint8_t keypairOut[64], uint8_t secretCryptoKey[32])
{
    Bits__memcpy(keypairOut,secretCryptoKey,32,"Sign.c",29);
    keypairOut[0] &= 248;
    keypairOut[31] &= 63;
    keypairOut[31] |= 64;
    ge_p3 A;
    crypto_sign_ed25519_ref10_ge_scalarmult_base(&A, keypairOut);
    crypto_sign_ed25519_ref10_ge_p3_tobytes(&keypairOut[32], &A);
}

void Sign_publicKeyFromKeyPair(uint8_t publicSigningKey[32], uint8_t keyPair[64])
{
    Bits__memcpy(publicSigningKey,&keyPair[32],32,"Sign.c",40);
}
...
void Sign_signMsg(uint8_t keyPair[64], struct Message* msg, struct Random* rand)
{
    ...
    Bits__memcpy(az,keyPair,32,"Sign.c",53);
    Random_bytes(rand, &az[32], 32);
    crypto_hash_sha512_ref(az,az,64);
    Bits__memcpy(az,keyPair,32,"Sign.c",56);
    az[0] &= 248;
    az[31] &= 63;
    az[31] |= 64;


    Message_push(msg, &az[32], 32, 
# 62 "crypto/Sign.c" 3 4
                                  ((void *)0)
# 62 "crypto/Sign.c"
                                      );
    crypto_hash_sha512_ref(r, msg->bytes, msg->length);


    Bits__memcpy(msg->bytes,&keyPair[32],32,"Sign.c",66);


    crypto_sign_ed25519_ref10_sc_reduce(r);
    crypto_sign_ed25519_ref10_ge_scalarmult_base(&R,r);
    Message_shift(msg, 32, 
# 71 "crypto/Sign.c" 3 4
                          ((void *)0)
# 71 "crypto/Sign.c"
                              );
    crypto_sign_ed25519_ref10_ge_p3_tobytes(msg->bytes,&R);

    crypto_hash_sha512_ref(hram, msg->bytes, msg->length);
    crypto_sign_ed25519_ref10_sc_reduce(hram);
    crypto_sign_ed25519_ref10_sc_muladd(&msg->bytes[32], hram, az, r);
}

int Sign_verifyMsg(uint8_t publicSigningKey[32], struct Message* msg)
{
    if (msg->length < 64) { return -1; }
    struct Allocator* alloc = Allocator__child((msg->alloc),"Sign.c",82);
    uint8_t* buff = Allocator__malloc((alloc),(msg->length),"Sign.c",83);
    unsigned long long ml = msg->length;
    int ret = crypto_sign_ed25519_ref10_open(buff, &ml, msg->bytes, msg->length, publicSigningKey);
    Allocator__free((alloc),"Sign.c",86);
    if (ret) {
        return -1;
    }
    Message_pop(msg, 
# 90 "crypto/Sign.c" 3 4
                    ((void *)0)
# 90 "crypto/Sign.c"
                        , 64, 
# 90 "crypto/Sign.c" 3 4
...
int Sign_publicSigningKeyToCurve25519(uint8_t curve25519keyOut[32], uint8_t publicSigningKey[32])
{
    ...
    if (crypto_sign_ed25519_ref10_ge_frombytes_negate_vartime(&A, publicSigningKey) != 0) {
        return -1;
    }
    crypto_sign_ed25519_ref10_fe_1(one_minus_y);
    crypto_sign_ed25519_ref10_fe_sub(one_minus_y, one_minus_y, A.Y);
    crypto_sign_ed25519_ref10_fe_invert(one_minus_y, one_minus_y);
    crypto_sign_ed25519_ref10_fe_1(x);
    crypto_sign_ed25519_ref10_fe_add(x, x, A.Y);
    crypto_sign_ed25519_ref10_fe_mul(x, x, one_minus_y);
    crypto_sign_ed25519_ref10_fe_tobytes(curve25519keyOut, x);
