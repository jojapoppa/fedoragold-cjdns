...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x04531106608084743;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x042351909315084235;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 20 "./io/ArrayReader.h" 2

struct Linker_x09675050978233839;
# 30 "./io/ArrayReader.h"
struct Reader* ArrayReader_new(const void* bufferToRead,
                               unsigned long length,
                               struct Allocator* allocator);
# 16 "io/ArrayReader.c" 2
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0739256376453387;
...
{
    struct ArrayReader_context* context = (__extension__ ({ __typeof__((struct ArrayReader_context*) reader) Identity_ptr = ((struct ArrayReader_context*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x5478941cb8301a27ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ArrayReader.c"), (29), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x5478941cb8301a27ull))"); } } while (0); Identity_ptr; }));
    ...
    Bits__memcpy(readInto,context->pointer,length,"ArrayReader.c",44);
...
{
    struct ArrayReader_context* context = (__extension__ ({ __typeof__((struct ArrayReader_context*) reader) Identity_ptr = ((struct ArrayReader_context*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x5478941cb8301a27ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ArrayReader.c"), (54), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x5478941cb8301a27ull))"); } } while (0); Identity_ptr; }));
...
struct Reader* ArrayReader_new(const void* bufferToRead,
                               ...
                               struct Allocator* alloc)
{
    struct ArrayReader_context* context = Allocator__clone((alloc),((&(struct ArrayReader_context) { .generic = { .read = read, .skip = skip }, .pointer = bufferToRead, .endPointer = (char*) bufferToRead + length })),sizeof(*((&(struct ArrayReader_context) { .generic = { .read = read, .skip = skip }, .pointer = bufferToRead, .endPointer = (char*) bufferToRead + length }))),"ArrayReader.c",64)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0x5478941cb8301a27ull);
