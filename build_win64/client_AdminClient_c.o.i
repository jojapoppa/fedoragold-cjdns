# 1 "client/AdminClient.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "client/AdminClient.c"
# 15 "client/AdminClient.c"
...
struct Linker_x05873723927413455;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x02944484330735704;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x005036076441016957;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x03233194879757555;
...
struct Linker_x07001976778722441;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x047131744272407516;
...
struct Linker_x0007262796301251706;
...
# 1 "./benc/Dict.h" 1
# 21 "./benc/Dict.h"
struct Linker_x03525398008124099;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x05889091765667627;
...
struct Linker_x06452350485671909;
...
struct Linker_x0667819337317183;
...
# 17 "client/AdminClient.c" 2
# 1 "./benc/serialization/standard/BencMessageWriter.h" 1
# 22 "./benc/serialization/standard/BencMessageWriter.h"
struct Linker_x018827432677938805;

void BencMessageWriter_write(Dict* toWrite, struct Message* msg, struct Except* eh);
# 18 "client/AdminClient.c" 2
# 1 "./benc/serialization/cloner/Cloner.h" 1
# 19 "./benc/serialization/cloner/Cloner.h"
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x08996868159660105;
...
# 20 "./benc/serialization/cloner/Cloner.h" 2



struct Linker_x034425909774835395;
...
{
    return ((orig) ? String_newBinary(orig->bytes, orig->len, alloc) : 
# 31 "./benc/serialization/cloner/Cloner.h" 3 4
          ((void *)0)
# 31 "./benc/serialization/cloner/Cloner.h"
          );
}
# 19 "client/AdminClient.c" 2



# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x020926675553898555;
# 32 "./util/Hex.h"
...
# 23 "client/AdminClient.c" 2
# 1 "./util/events/Timeout.h" 1
# 20 "./util/events/Timeout.h"
struct Linker_x013351258965677015;
...
# 31 "client/AdminClient.c" 2


# 32 "client/AdminClient.c"
struct Request;
...
static inline struct Map_OfRequestByHandle* Map_OfRequestByHandle_new(struct Allocator* allocator)
{
    return Allocator__clone((allocator),((&(struct Map_OfRequestByHandle) { .allocator = allocator })),sizeof(*((&(struct Map_OfRequestByHandle) { .allocator = allocator }))),"Map.h",85)

       ;
}
# 110 "./util/Map.h"
static inline int Map_OfRequestByHandle_indexForHandle(uint32_t handle, struct Map_OfRequestByHandle* map)
{
    uint32_t base = 0;
...
static inline int Map_OfRequestByHandle_remove(int index, struct Map_OfRequestByHandle* map)
{
...
# 146 "./util/Map.h"
            ...
            __builtin_memmove(&map->values[index],&map->values[index + 1],(map->count - index - 1) * sizeof(struct Request*))
...
static inline int Map_OfRequestByHandle_put(struct Request** value,
                                    struct Map_OfRequestByHandle* map)
...
# 190 "./util/Map.h"
        ...
        map->values = Allocator__realloc((map->allocator),(map->values),(sizeof(struct Request*) * (map->count + 10)),"Map.h",195)
    ...
    Bits__memcpy(&map->values[i],value,sizeof(struct Request*),"Map.h",220);

    return i;
}
# 60 "client/AdminClient.c" 2
                         ...
                         struct Allocator* alloc)
{
    ...
    String* hashHex = String_newBinary(
# 80 "client/AdminClient.c" 3 4
                                      ((void *)0)
# 80 "client/AdminClient.c"
                                          , 64, alloc);
    uint8_t passAndCookie[64];
    uint32_t cookie = (cookieStr != 
# 82 "client/AdminClient.c" 3 4
                                   ((void *)0)
# 82 "client/AdminClient.c"
...
# 82 "client/AdminClient.c"
    ...
    struct Message* msg = Message_new(0, 1023, alloc);
    BencMessageWriter_write(message, msg, 
# 93 "client/AdminClient.c" 3 4
                                         ((void *)0)
# 93 "client/AdminClient.c"
                                             );


    crypto_hash_sha256_ref(hash, msg->bytes, msg->length);
...
static void done(struct Request* req, enum AdminClient_Error err)
{
    req->res.err = err;
    req->callback(req);
    Allocator__free((req->timeoutAlloc),"AdminClient.c",108);
}
...
static __attribute__ ((warn_unused_result)) struct Iface* receiveMessage(struct Message* msg, struct Iface* addrIface)
{
    struct Context* ctx = (__extension__ ({ const __typeof__(((struct Context*)0)->addrIface)*__mptr = (addrIface); (__extension__ ({ __typeof__((struct Context*)((char*)__mptr - 
# 118 "client/AdminClient.c" 3 4
                         __builtin_offsetof (
# 118 "client/AdminClient.c"
                         struct Context
# 118 "client/AdminClient.c" 3 4
                         , 
# 118 "client/AdminClient.c"
...
# 118 "client/AdminClient.c"
                         )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (118), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull))"); } } while (0); Identity_ptr; })); }));

    struct Sockaddr_storage source;
    Message_pop(msg, &source, ctx->targetAddr->addrLen, 
# 121 "client/AdminClient.c" 3 4
                                                       ((void *)0)
# 121 "client/AdminClient.c"
                                                           );
    if (__builtin_memcmp(&source,ctx->targetAddr,ctx->targetAddr->addrLen)) {
        do { if (ctx->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_INFO, "AdminClient.c", 123, "Got spurious message from [%s], expecting messages from [%s]", Sockaddr_print(&source.addr, msg->alloc), Sockaddr_print(ctx->targetAddr, msg->alloc)); } } while (0)

                                                             ;
        return 
# 126 "client/AdminClient.c" 3 4
              ((void *)0)
# 126 "client/AdminClient.c"
                  ;
    }



    struct Allocator* alloc = Allocator__child((msg->alloc),"AdminClient.c",131);

    int origLen = msg->length;
    Dict* d = 
# 134 "client/AdminClient.c" 3 4
             ((void *)0)
# 134 "client/AdminClient.c"
                 ;
    char* err = BencMessageReader_readNoExcept(msg, alloc, &d);
    if (err) { return 
# 136 "client/AdminClient.c" 3 4
                     ((void *)0)
# 136 "client/AdminClient.c"
                         ; }
    Message_shift(msg, origLen, 
# 137 "client/AdminClient.c" 3 4
                               ((void *)0)
# 137 "client/AdminClient.c"
                                   );

    String* txid = Dict_getString(d, (&(String) { .bytes = "txid", .len = CString_strlen("txid") }));
    if (!txid || txid->len != 8) { return 
# 140 "client/AdminClient.c" 3 4
                                         ((void *)0)
# 140 "client/AdminClient.c"
                                             ; }
    ...
    if (idx < 0) { return 
# 146 "client/AdminClient.c" 3 4
                         ((void *)0)
# 146 "client/AdminClient.c"
                             ; }

    struct Request* req = ctx->outstandingRequests.values[idx];


    Allocator__adopt((req->promise->alloc),(alloc),"AdminClient.c",151);

    req->res.responseDict = d;

    int len =
        (msg->length > 1023) ? 1023 : msg->length;
    __builtin_memset(req->res.messageBytes,0,1023);
    Bits__memcpy(req->res.messageBytes,msg->bytes,len,"AdminClient.c",158);
    done(req, AdminClient_Error_NONE);
    return 
# 160 "client/AdminClient.c" 3 4
          ((void *)0)
# 160 "client/AdminClient.c"
              ;
}

static int requestOnFree(struct Allocator_OnFreeJob* job)
{
    struct Request* req = (__extension__ ({ __typeof__((struct Request*) job->userData) Identity_ptr = ((struct Request*) job->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (165), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull))"); } } while (0); Identity_ptr; }));
...
static struct Request* sendRaw(Dict* messageDict,
                               ...
                               AdminClient_RespHandler callback)
{
    struct Allocator* reqAlloc = Allocator__child((promise->alloc),"AdminClient.c",179);
    struct Request* req = Allocator__clone((reqAlloc),((&(struct Request) { .alloc = reqAlloc, .ctx = ctx, .promise = promise })),sizeof(*((&(struct Request) { .alloc = reqAlloc, .ctx = ctx, .promise = promise }))),"AdminClient.c",180)



       ;
    (req)->Identity_verifier = ((unsigned long) 0x2999522752f9d284ull);

    int idx = Map_OfRequestByHandle_put(&req, &ctx->outstandingRequests);
    req->handle = ctx->outstandingRequests.handles[idx];

    String* id = String_newBinary(
# 190 "client/AdminClient.c" 3 4
                                 ((void *)0)
# 190 "client/AdminClient.c"
                                     , 8, req->alloc);
    Hex_encode(id->bytes, 8, (int8_t*) &req->handle, 4);
    Dict_putString(messageDict, String_new("txid", req->alloc), id, req->alloc);

    if (cookie) {
        do { if (!((!calculateAuth(messageDict, ctx->password, cookie, req->alloc)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (195), "(!calculateAuth(messageDict, ctx->password, cookie, req->alloc))"); } } while (0);
    }

    struct Allocator* child = Allocator__child((req->alloc),"AdminClient.c",198);
    struct Message* msg = Message_new(0, 1023 + 256, child);
    BencMessageWriter_write(messageDict, msg, 
# 200 "client/AdminClient.c" 3 4
                                             ((void *)0)
# 200 "client/AdminClient.c"
                                                 );

    req->timeoutAlloc = Allocator__child((req->alloc),"AdminClient.c",202);
    req->timeout = Timeout__setTimeout((timeout), (req), (ctx->pub.millisecondsToWait), (ctx->eventBase), (req->timeoutAlloc), "AdminClient.c", 203)



                                                        ;
    Allocator__onFree((req->timeoutAlloc), (requestOnFree), (req), "AdminClient.c", 208);

    req->callback = callback;

    Message_push(msg, ctx->targetAddr, ctx->targetAddr->addrLen, 
# 212 "client/AdminClient.c" 3 4
                                                                ((void *)0)
# 212 "client/AdminClient.c"
                                                                    );
    Iface_send(&ctx->addrIface, msg);
    Allocator__free((child),"AdminClient.c",214);
...
static void requestCallback(struct Request* req)
{
    ...
    Allocator__free((req->promise->alloc),"AdminClient.c",224);
}
...
static void cookieCallback(struct Request* req)
{
    ...
    String* cookie = Dict_getString(req->res.responseDict, (&(String) { .bytes = "cookie", .len = CString_strlen("cookie") }));
    if (!cookie) {
    ...
    Allocator__free((req->alloc),"AdminClient.c",243);
}
...
static struct AdminClient_Promise* doCall(Dict* message,
                                          ...
                                          struct Allocator* alloc)
{
    struct Allocator* promiseAlloc = Allocator__child((alloc),"AdminClient.c",250);
    struct AdminClient_Promise* promise =
        Allocator__calloc((promiseAlloc),(sizeof(struct AdminClient_Promise)),(1),"AdminClient.c",252);
    promise->alloc = promiseAlloc;

    Dict gc = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "q", .len = CString_strlen("q") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = "cookie", .len = CString_strlen("cookie") }) }), .next = 
# 255 "client/AdminClient.c" 3 4
             ((void *)0) 
# 255 "client/AdminClient.c"
             });
    struct Request* req = sendRaw(&gc, promise, ctx, 
# 256 "client/AdminClient.c" 3 4
                                                    ((void *)0)
# 256 "client/AdminClient.c"
                                                        , cookieCallback);
...
struct AdminClient_Promise* AdminClient_rpcCall(String* function,
                                                ...
                                                struct Allocator* alloc)
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) client) Identity_ptr = ((struct Context*) client); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AdminClient.c"), (266), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x2999522752f9d284ull))"); } } while (0); Identity_ptr; }));
    Dict a = (args) ? *args : 
# 267 "client/AdminClient.c" 3 4
                             ((void *)0)
# 267 "client/AdminClient.c"
                                 ;
    Dict message = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "q", .len = CString_strlen("q") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = "auth", .len = CString_strlen("auth") }) }), .next = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "aq", .len = CString_strlen("aq") }), .val = (&(Object) { .type = Object_STRING, .as.string = function }), .next = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "args", .len = CString_strlen("args") }), .val = (&(Object) { .type = Object_DICT, .as.dictionary = &a }), .next = 
# 268 "client/AdminClient.c" 3 4
                  ((void *)0) 
# 268 "client/AdminClient.c"
                  }) }) })
...
struct AdminClient* AdminClient_new(struct AddrIface* ai,
                                    ...
                                    struct Allocator* alloc)
{
    struct Context* context = Allocator__clone((alloc),((&(struct Context) { .eventBase = eventBase, .logger = logger, .password = adminPassword, .pub = { .millisecondsToWait = 5000, }, .outstandingRequests = { .allocator = alloc }, .alloc = alloc })),sizeof(*((&(struct Context) { .eventBase = eventBase, .logger = logger, .password = adminPassword, .pub = { .millisecondsToWait = 5000, }, .outstandingRequests = { .allocator = alloc }, .alloc = alloc }))),"AdminClient.c",307)
# 318 "client/AdminClient.c"
       ;
    context->addrIface.send = receiveMessage;
    (context)->Identity_verifier = ((unsigned long) 0x2999522752f9d284ull);
    ...
    if (Sockaddr_getFamily(context->targetAddr) == Sockaddr_AF_INET) {
        ...
        if (Bits_isZero(addrBytes, len)) {
            ...
            uint32_t loopback = ((uint32_t)__builtin_bswap32(((uint32_t)(0x7f000001))));
            Bits__memcpy(addrBytes,&loopback,4,"AdminClient.c",329);
        }
    }
    do { if (logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(logger, Log_Level_DEBUG, "AdminClient.c", 332, "Connecting to [%s]", Sockaddr_print(context->targetAddr, alloc)); } } while (0);
