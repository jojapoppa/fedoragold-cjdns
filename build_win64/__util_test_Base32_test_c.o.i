# 1 "./util/test/Base32_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/test/Base32_test.c"
# 15 "./util/test/Base32_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09485258870867186;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x06784721981205852;
...
struct Linker_x03207242901682046;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x005941909054378236;
...
struct Linker_x0004916159379011686;
...
struct Linker_x019200691253732893;
...
# 22 "./util/test/Base32_test.c" 2


# 23 "./util/test/Base32_test.c"
int Base32_test_main(int argc, char** argv);int Base32_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((20000),"Base32_test.c",25);
    struct Random* rand = Random_new(alloc, 
# 26 "./util/test/Base32_test.c" 3 4
                                           ((void *)0)
# 26 "./util/test/Base32_test.c"
                                               , 
# 26 "./util/test/Base32_test.c" 3 4
                                                 ((void *)0)
# 26 "./util/test/Base32_test.c"
    ...
    uint8_t base32[64];
    __builtin_memset(base32,0,64);

    do { if (!((Base32_encode(base32, 64, bytes, 32) == 52))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Base32_test.c"), (34), "(Base32_encode(base32, 64, bytes, 32) == 52)"); } } while (0);



    uint8_t bytes2[32];
    do { if (!((Base32_decode(bytes2, 32, base32, 52) == 32))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Base32_test.c"), (39), "(Base32_decode(bytes2, 32, base32, 52) == 32)"); } } while (0);

    do { if (!((__builtin_memcmp(bytes,bytes2,32) == 0))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Base32_test.c"), (41), "(__builtin_memcmp(bytes,bytes2,32) == 0)"); } } while (0);

    Allocator__free((alloc),"Base32_test.c",43);
    return 0;
}
