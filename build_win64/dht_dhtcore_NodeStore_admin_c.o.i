...
struct Linker_x07664997142502237;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x019234696788366357;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x0005619713626995537;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x045870892753755976;
...
struct Linker_x00945239610826254;
...
struct Linker_x021656389970641432;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x04737218111748436;
...
struct Linker_x06029149979889337;
...
struct Linker_x0503886505975137;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x09255316248483212;
...
struct Linker_x05526020671248342;
...
struct Linker_x07806491651242435;
...
struct Linker_x02733855163405845;
...
struct Linker_x08805005085452342;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x04822192648440795;
# 34 "./dht/Address.h"
struct Address
{
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x012343627555418069;
...
struct Linker_x03455537916045004;
...
struct Linker_x00345722231610881;
...
struct Linker_x0024271335327057164;
...
struct Linker_x07872166945910457;
...
struct Linker_x013482935629656345;
...
# 19 "./util/AddrTools.h"
struct Linker_x0654142906840623;
...
struct Linker_x0022493538020962767;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (39), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (115), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (194), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (260), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (304), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
    Dict* response = Dict_new(requestAlloc);
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NodeStore_admin.c"), (313), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x0d80ade747a02ec4ull))"); } } while (0); Identity_ptr; }));
                              ...
                              struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .alloc = alloc, .store = nodeStore })),sizeof(*((&(struct Context) { .admin = admin, .alloc = alloc, .store = nodeStore }))),"NodeStore_admin.c",325)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x0d80ade747a02ec4ull);
