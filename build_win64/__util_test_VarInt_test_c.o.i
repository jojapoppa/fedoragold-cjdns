# 1 "./util/test/VarInt_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/test/VarInt_test.c"
# 15 "./util/test/VarInt_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x05303882707502834;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x07387067445689892;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x02222065086691356;
...
struct Linker_x04625728755776146;
...
struct Linker_x05119122652307755;
...
# 17 "./util/test/VarInt_test.c" 2
# 1 "./util/VarInt.h" 1
# 18 "./util/VarInt.h"
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x021061954910297875;
...
# 22 "./util/test/VarInt_test.c" 2




# 25 "./util/test/VarInt_test.c"
static void fidelityTest()
{
    struct Allocator* alloc = MallocAllocator__new((1<<20),"VarInt_test.c",27);
    struct Random* rand = Random_new(alloc, 
# 28 "./util/test/VarInt_test.c" 3 4
                                           ((void *)0)
# 28 "./util/test/VarInt_test.c"
                                               , 
# 28 "./util/test/VarInt_test.c" 3 4
                                                 ((void *)0)
# 28 "./util/test/VarInt_test.c"
                                                     );
    uint64_t* buf = Allocator__malloc((alloc),(1024*8),"VarInt_test.c",29);
    Random_bytes(rand, (uint8_t*) buf, 1024*8);
    ...
    uint8_t* buf2 = Allocator__malloc((alloc),(1024*8),"VarInt_test.c",43);
    struct VarInt_Iter iter;
    VarInt_mk(&iter, buf2, 1024*8);
    VarInt_toEnd(&iter);

    for (int i = 0; i < 1024; i++) { do { if (!((!VarInt_push(&iter, buf[i])))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (48), "(!VarInt_push(&iter, buf[i]))"); } } while (0); }
    do { if (!((iter.ptr > buf2))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (49), "(iter.ptr > buf2)"); } } while (0);

    for (int i = 1024 - 1; i >= 0; i--) {
        uint64_t x = ~0;
        do { if (!((!VarInt_pop(&iter, &x)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (53), "(!VarInt_pop(&iter, &x))"); } } while (0);
        do { if (!((buf[i] == x))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (54), "(buf[i] == x)"); } } while (0);
    }

    do { if (!((iter.ptr == iter.end))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (57), "(iter.ptr == iter.end)"); } } while (0);

    Allocator__free((alloc),"VarInt_test.c",59);
}
...
static inline void test(uint8_t* bytes, int len, uint64_t num, uint8_t* buf)
{
    ...
    uint64_t out = ~0;
    do { if (!((!VarInt_pop(&iter, &out)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (66), "(!VarInt_pop(&iter, &out))"); } } while (0);
    do { if (!((out == num))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (67), "(out == num)"); } } while (0);
    do { if (!((iter.ptr == iter.end))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (68), "(iter.ptr == iter.end)"); } } while (0);

    buf[0] = buf[len+1] = 0xee;
    struct VarInt_Iter iter2 = { .ptr = &buf[len+1], .end = &buf[len+1], .start = &buf[1] };
    if (len < 9) { do { if (!((VarInt_push(&iter2, 0xf000000000000000ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (72), "(VarInt_push(&iter2, 0xf000000000000000ull))"); } } while (0); }
    if (len < 5) { do { if (!((VarInt_push(&iter2, 0xf0000000)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (73), "(VarInt_push(&iter2, 0xf0000000))"); } } while (0); }
    if (len < 3) { do { if (!((VarInt_push(&iter2, 0xf000)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (74), "(VarInt_push(&iter2, 0xf000))"); } } while (0); }
    do { if (!((!VarInt_push(&iter2, num)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (75), "(!VarInt_push(&iter2, num))"); } } while (0);
    do { if (!((iter2.ptr == iter2.start))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (76), "(iter2.ptr == iter2.start)"); } } while (0);
    do { if (!((buf[0] == 0xee))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (77), "(buf[0] == 0xee)"); } } while (0);
    do { if (!((buf[len+1] == 0xee))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("VarInt_test.c"), (78), "(buf[len+1] == 0xee)"); } } while (0);
}
...
{
    do { uint8_t data[sizeof("\x00")+1] = {0}; test("\x00", sizeof("\x00")-1, 0x00, data); } while (0);
    do { uint8_t data[sizeof("\xAC")+1] = {0}; test("\xAC", sizeof("\xAC")-1, 0xAC, data); } while (0);
    do { uint8_t data[sizeof("\xFD\xAB\xCD")+1] = {0}; test("\xFD\xAB\xCD", sizeof("\xFD\xAB\xCD")-1, 0xABCD, data); } while (0);
    do { uint8_t data[sizeof("\xFE\xAB\xCD\xEF\x01")+1] = {0}; test("\xFE\xAB\xCD\xEF\x01", sizeof("\xFE\xAB\xCD\xEF\x01")-1, 0xABCDEF01, data); } while (0);
    do { uint8_t data[sizeof("\xFF\xAB\xCD\xEF\x01\x23\x45\x67\x89")+1] = {0}; test("\xFF\xAB\xCD\xEF\x01\x23\x45\x67\x89", sizeof("\xFF\xAB\xCD\xEF\x01\x23\x45\x67\x89")-1, 0xABCDEF0123456789ull, data); } while (0);
}

int VarInt_test_main(int argc, char** argv);int VarInt_test_main(int argc, char** argv)
{
    simpleTest();
    fidelityTest();
    return 0;
