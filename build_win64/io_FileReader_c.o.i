...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07579795415660184;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05788164834501828;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 20 "./io/FileReader.h" 2

struct Linker_x013760433126158933;
...
{
    struct FileReader* context = (__extension__ ({ __typeof__((struct FileReader*) reader) Identity_ptr = ((struct FileReader*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xffbc3a67d16788c5ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileReader.c"), (33), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xffbc3a67d16788c5ull))"); } } while (0); Identity_ptr; }));
...
{
    struct FileReader* context = (__extension__ ({ __typeof__((struct FileReader*) reader) Identity_ptr = ((struct FileReader*) reader); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xffbc3a67d16788c5ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileReader.c"), (58), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xffbc3a67d16788c5ull))"); } } while (0); Identity_ptr; }));
    ...
    size_t amount;
    while ((amount = (length > 256) ? 256 : length) && !context->failed) {
...
{
    struct FileReader* context = Allocator__clone((allocator),((&(struct FileReader) { .generic = { .read = read, .skip = skip }, .toRead = toRead })),sizeof(*((&(struct FileReader) { .generic = { .read = read, .skip = skip }, .toRead = toRead }))),"FileReader.c",74)
       ...
       ;
    (context)->Identity_verifier = ((unsigned long) 0xffbc3a67d16788c5ull);
