# 1 "dht/dhtcore/RouterModule_admin.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "dht/dhtcore/RouterModule_admin.c"
# 15 "dht/dhtcore/RouterModule_admin.c"
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
struct Linker_x05701690724087158;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x01368299165747231;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x03782835821156858;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x009462498104198458;
...
struct Linker_x08514692985602854;
...
struct Linker_x09489370080420167;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x025503058972466053;
...
struct Linker_x08510586530431408;
...
struct Linker_x025022383266478365;
...
struct Admin* Admin_new(struct AddrIface* ai,
                        ...
                        String* password);
# 16 "dht/dhtcore/RouterModule_admin.c" 2

# 1 "./benc/String.h" 1
# 20 "./benc/String.h"
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x06435103717282176;
...
struct Linker_x0543678652320146;
...
# 18 "dht/dhtcore/RouterModule_admin.c" 2
# 1 "./benc/Int.h" 1
# 19 "dht/dhtcore/RouterModule_admin.c" 2
# 1 "./dht/dhtcore/Node.h" 1
# 22 "./dht/dhtcore/Node.h"
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x05489655022862068;
# 34 "./dht/Address.h"
struct Address
{
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x01711096702647532;
...
struct Linker_x031383689403130033;
...
struct Linker_x038577809952515274;
...
# 20 "dht/dhtcore/RouterModule_admin.c" 2
...
struct Linker_x039294781533401824;
...
struct Linker_x00437943910990406;
...
struct Linker_x0601790795729354;
...
struct Linker_x08361069256802445;
...
struct Linker_x09534632869585027;
...
struct Linker_x006660333863389045;
...
void RouterModule_peerIsReachable(uint64_t pathToPeer,
                                  ...
                                  struct RouterModule* module);
# 21 "dht/dhtcore/RouterModule_admin.c" 2
...
struct Linker_x05373087457036976;
...
# 26 "dht/dhtcore/RouterModule_admin.c" 2

# 1 "./util/AddrTools.h" 1
# 19 "./util/AddrTools.h"
struct Linker_x022983094111513247;
...
# 28 "dht/dhtcore/RouterModule_admin.c" 2
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x023008167118967715;
# 32 "./util/Hex.h"
...
# 29 "dht/dhtcore/RouterModule_admin.c" 2
...
static void lookup(Dict* args, void* vcontext, String* txid, struct Allocator* requestAlloc)
{
    ...
    String* addrStr = Dict_getString(args, (&(String) { .bytes = "address", .len = CString_strlen("address") }));
    char* err = 
# 42 "dht/dhtcore/RouterModule_admin.c" 3 4
               ((void *)0)
# 42 "dht/dhtcore/RouterModule_admin.c"
                   ;
    uint8_t addr[16];
    ...
    if (addrStr->len != 39) {
        ...
        if (!n) {
            result = "not found";
        } else if (__builtin_memcmp(addr,n->address.ip6.bytes,16)) {
            Address_print(resultBuff, &n->address);
        } else {
            AddrTools_printPath(resultBuff, n->address.path);
        }
    }
    Dict response = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = (err) ? err : "none", .len = CString_strlen((err) ? err : "none") }) }), .next = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "result", .len = CString_strlen("result") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = result, .len = CString_strlen(result) }) }), .next = 
# 60 "dht/dhtcore/RouterModule_admin.c" 3 4
                   ((void *)0) 
# 60 "dht/dhtcore/RouterModule_admin.c"
                   }) })
...
static void pingResponse(struct RouterModule_Promise* promise,
                         ...
                         Dict* responseDict)
{
    struct Ping* ping = (__extension__ ({ __typeof__((struct Ping*)promise->userData) Identity_ptr = ((struct Ping*)promise->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (80), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    ...
    if (versionBin && versionBin->len == 20) {
        String* versionStr = String_newBinary(
# 86 "dht/dhtcore/RouterModule_admin.c" 3 4
                                             ((void *)0)
# 86 "dht/dhtcore/RouterModule_admin.c"
                                                 , 40, tempAlloc);
        Hex_encode(versionStr->bytes, 40, versionBin->bytes, 20);
        Dict_putString(resp, String_new("version", tempAlloc), versionStr, tempAlloc);
    } else {
        Dict_putString(resp, String_new("version", tempAlloc), String_new("unknown", tempAlloc), tempAlloc);
    }

    String* result = (responseDict) ? (&(String) { .bytes = "pong", .len = CString_strlen("pong") }) : (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") });
    Dict_putString(resp, String_new("result", tempAlloc), result, tempAlloc);

    int64_t* protocolVersion = Dict_getInt(responseDict, CJDHTConstants_PROTOCOL);
    if (protocolVersion) {
        Dict_putInt(resp, String_new("protocol", tempAlloc), *protocolVersion, tempAlloc);
    }

    Dict_putInt(resp, String_new("ms", tempAlloc), lag, tempAlloc);

    if (from) {
        uint8_t fromStr[60] = "";
        Address_print(fromStr, from);
        Dict_putString(resp, String_new("from", tempAlloc), String_new(fromStr, tempAlloc), tempAlloc);
        String* addr = Address_toString(from, tempAlloc);
        Dict_putString(resp, String_new("addr", tempAlloc), addr, tempAlloc);
    }

    Dict_putString(resp, String_new("deprecation", tempAlloc), String_new("from,protocol,version will soon be removed", tempAlloc), tempAlloc)
                                                                ;
...
# 122 "dht/dhtcore/RouterModule_admin.c" 3 4
                           _Bool 
# 122 "dht/dhtcore/RouterModule_admin.c"
                                splicePath)
{
    struct Ping* ping = (__extension__ ({ __typeof__((struct Ping*)promise->userData) Identity_ptr = ((struct Ping*)promise->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (124), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    Dict* out = Dict_new(promise->alloc);
    String* result = (responseDict) ? name : (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") });
    Dict_putString(out, String_new("result", promise->alloc), result, promise->alloc);

    if (responseDict) {
        struct Address_List* addrs =
            ReplySerializer_parse(from, responseDict, 
# 131 "dht/dhtcore/RouterModule_admin.c" 3 4
                                                     ((void *)0)
# 131 "dht/dhtcore/RouterModule_admin.c"
                                                         , splicePath, promise->alloc);
    ...
    Dict_putInt(out, String_new("ms", promise->alloc), lag, promise->alloc);

    Dict_putString(out, String_new("error", promise->alloc), String_new("none", promise->alloc), promise->alloc);
...
static void getPeersResponse(struct RouterModule_Promise* promise,
                             ...
                             Dict* responseDict)
{
    genericResponse(promise, lag, from, responseDict, (&(String) { .bytes = "peers", .len = CString_strlen("peers") }), 
# 164 "dht/dhtcore/RouterModule_admin.c" 3 4
                                                                            0
# 164 "dht/dhtcore/RouterModule_admin.c"
                                                                                 );
}
...
static void findNodeResponse(struct RouterModule_Promise* promise,
                             ...
                             Dict* responseDict)
{
    genericResponse(promise, lag, from, responseDict, (&(String) { .bytes = "nodes", .len = CString_strlen("nodes") }), 
# 172 "dht/dhtcore/RouterModule_admin.c" 3 4
                                                                            1
# 172 "dht/dhtcore/RouterModule_admin.c"
                                                                                );
}
                               ...
                               struct Allocator* alloc)
{
    ...
    if (pathStr->len == 19 && !AddrTools_parsePath(&addr.path, pathStr->bytes)) {
        ...
        if (!nl) {
            *errOut = "not_found";
            return 
# 186 "dht/dhtcore/RouterModule_admin.c" 3 4
                  ((void *)0)
# 186 "dht/dhtcore/RouterModule_admin.c"
                      ;
        } else {
            Bits__memcpy(&addr,&nl->child->address,sizeof(struct Address),"RouterModule_admin.c",188);
        }
    } else if (!AddrTools_parseIp(addr.ip6.bytes, pathStr->bytes)) {
        struct Node_Two* n = Router_lookup(ctx->router, addr.ip6.bytes);
        if (!n || __builtin_memcmp(addr.ip6.bytes,n->address.ip6.bytes,16)) {
            *errOut = "not_found";
            return 
# 194 "dht/dhtcore/RouterModule_admin.c" 3 4
                  ((void *)0)
# 194 "dht/dhtcore/RouterModule_admin.c"
                      ;
        } else {
            Bits__memcpy(&addr,&n->address,sizeof(struct Address),"RouterModule_admin.c",196);
        }
    } else {
        ...
        *errOut = "parse_path";
        return 
# 202 "dht/dhtcore/RouterModule_admin.c" 3 4
              ((void *)0)
# 202 "dht/dhtcore/RouterModule_admin.c"
                  ;
    }

    return Allocator__clone((alloc),(&addr),sizeof(*(&addr)),"RouterModule_admin.c",205);
}

static void pingNode(Dict* args, void* vctx, String* txid, struct Allocator* requestAlloc)
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (210), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    String* pathStr = Dict_getString(args, (&(String) { .bytes = "path", .len = CString_strlen("path") }));
    int64_t* timeoutPtr = Dict_getInt(args, (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") }));
    uint32_t timeout = (timeoutPtr && *timeoutPtr > 0) ? *timeoutPtr : 0;

    char* err = 
# 215 "dht/dhtcore/RouterModule_admin.c" 3 4
               ((void *)0)
# 215 "dht/dhtcore/RouterModule_admin.c"
                   ;
    struct Address* addr = getNode(pathStr, ctx, &err, requestAlloc);

    if (err) {
        Dict errDict = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = err, .len = CString_strlen(err) }) }), .next = 
# 219 "dht/dhtcore/RouterModule_admin.c" 3 4
                      ((void *)0) 
# 219 "dht/dhtcore/RouterModule_admin.c"
                      });
    ...
    struct Ping* ping = Allocator__calloc((rp->alloc),(sizeof(struct Ping)),(1),"RouterModule_admin.c",226);
    (ping)->Identity_verifier = ((unsigned long) 0x7b6aaccbabadb498ull);
    ping->txid = ((txid) ? String_newBinary(txid->bytes, txid->len, rp->alloc) : 
# 228 "dht/dhtcore/RouterModule_admin.c" 3 4
                ((void *)0)
# 228 "dht/dhtcore/RouterModule_admin.c"
                );
    ping->rp = rp;
    ping->ctx = ctx;
    rp->userData = ping;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (237), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    String* nearbyLabelStr = Dict_getString(args, (&(String) { .bytes = "nearbyPath", .len = CString_strlen("nearbyPath") }));
    String* pathStr = Dict_getString(args, (&(String) { .bytes = "path", .len = CString_strlen("path") }));
    int64_t* timeoutPtr = Dict_getInt(args, (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") }));
    uint32_t timeout = (timeoutPtr && *timeoutPtr > 0) ? *timeoutPtr : 0;

    char* err = 
# 243 "dht/dhtcore/RouterModule_admin.c" 3 4
               ((void *)0)
# 243 "dht/dhtcore/RouterModule_admin.c"
                   ;
    struct Address* addr = getNode(pathStr, ctx, &err, requestAlloc);
    ...
    if (err) {
        Dict errDict = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = err, .len = CString_strlen(err) }) }), .next = 
# 254 "dht/dhtcore/RouterModule_admin.c" 3 4
                      ((void *)0) 
# 254 "dht/dhtcore/RouterModule_admin.c"
                      });
        Admin_sendMessage(&errDict, txid, ctx->admin);
        return;
    }
    ...
    struct Ping* ping = Allocator__calloc((rp->alloc),(sizeof(struct Ping)),(1),"RouterModule_admin.c",262);
    (ping)->Identity_verifier = ((unsigned long) 0x7b6aaccbabadb498ull);
    ping->txid = ((txid) ? String_newBinary(txid->bytes, txid->len, rp->alloc) : 
# 264 "dht/dhtcore/RouterModule_admin.c" 3 4
                ((void *)0)
# 264 "dht/dhtcore/RouterModule_admin.c"
                );
    ping->rp = rp;
    ping->ctx = ctx;
    rp->userData = ping;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (273), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    String* nodeToQueryStr = Dict_getString(args, (&(String) { .bytes = "nodeToQuery", .len = CString_strlen("nodeToQuery") }));
    String* targetStr = Dict_getString(args, (&(String) { .bytes = "target", .len = CString_strlen("target") }));
    int64_t* timeoutPtr = Dict_getInt(args, (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") }));
    uint32_t timeout = (timeoutPtr && *timeoutPtr > 0) ? *timeoutPtr : 0;

    char* err = 
# 279 "dht/dhtcore/RouterModule_admin.c" 3 4
               ((void *)0)
# 279 "dht/dhtcore/RouterModule_admin.c"
                   ;
    ...
    if (err) {
        Dict errDict = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = err, .len = CString_strlen(err) }) }), .next = 
# 290 "dht/dhtcore/RouterModule_admin.c" 3 4
                      ((void *)0) 
# 290 "dht/dhtcore/RouterModule_admin.c"
                      });
        Admin_sendMessage(&errDict, txid, ctx->admin);
        return;
    }
    ...
    struct Ping* ping = Allocator__calloc((rp->alloc),(sizeof(struct Ping)),(1),"RouterModule_admin.c",298);
    (ping)->Identity_verifier = ((unsigned long) 0x7b6aaccbabadb498ull);
    ping->txid = ((txid) ? String_newBinary(txid->bytes, txid->len, rp->alloc) : 
# 300 "dht/dhtcore/RouterModule_admin.c" 3 4
                ((void *)0)
# 300 "dht/dhtcore/RouterModule_admin.c"
                );
    ping->rp = rp;
    ping->ctx = ctx;
    rp->userData = ping;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RouterModule_admin.c"), (309), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x7b6aaccbabadb498ull))"); } } while (0); Identity_ptr; }));
    String* nodeToQueryStr = Dict_getString(args, (&(String) { .bytes = "nodeToQuery", .len = CString_strlen("nodeToQuery") }));
    String* targetStr = Dict_getString(args, (&(String) { .bytes = "target", .len = CString_strlen("target") }));
    int64_t* timeoutPtr = Dict_getInt(args, (&(String) { .bytes = "timeout", .len = CString_strlen("timeout") }));
    uint32_t timeout = (timeoutPtr && *timeoutPtr > 0) ? *timeoutPtr : 0;
    ...
    char* err = 
# 315 "dht/dhtcore/RouterModule_admin.c" 3 4
               ((void *)0)
# 315 "dht/dhtcore/RouterModule_admin.c"
                   ;
    struct Address* nodeToQuery = getNode(nodeToQueryStr, ctx, &err, requestAlloc);
    uint8_t target[16];
    ...
    if (err) {
        Dict errDict = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = err, .len = CString_strlen(err) }) }), .next = 
# 322 "dht/dhtcore/RouterModule_admin.c" 3 4
                      ((void *)0) 
# 322 "dht/dhtcore/RouterModule_admin.c"
                      });
        Admin_sendMessage(&errDict, txid, ctx->admin);
        return;
    }
    ...
    struct Ping* ping = Allocator__calloc((rp->alloc),(sizeof(struct Ping)),(1),"RouterModule_admin.c",330);
    (ping)->Identity_verifier = ((unsigned long) 0x7b6aaccbabadb498ull);
    ping->txid = ((txid) ? String_newBinary(txid->bytes, txid->len, rp->alloc) : 
# 332 "dht/dhtcore/RouterModule_admin.c" 3 4
                ((void *)0)
# 332 "dht/dhtcore/RouterModule_admin.c"
                );
    ping->rp = rp;
    ping->ctx = ctx;
    rp->userData = ping;
                                 ...
                                 struct Allocator* alloc)
{
    ...
    alloc = Allocator__child((alloc),"RouterModule_admin.c",345);
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .allocator = alloc, .module = module, .router = router })),sizeof(*((&(struct Context) { .admin = admin, .allocator = alloc, .module = module, .router = router }))),"RouterModule_admin.c",346)




       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x7b6aaccbabadb498ull);

    Admin_registerFunctionWithArgCount( "RouterModule_nextHop", nextHop, ctx, 
# 354 "dht/dhtcore/RouterModule_admin.c" 3 4
   1
# 354 "dht/dhtcore/RouterModule_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "nodeToQuery", .required = 1, .type = "String" }, { .name = "target", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "nodeToQuery", .required = 1, .type = "String" }, { .name = "target", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" } })) / sizeof(struct Admin_FunctionArg)), admin)
    ...
    Admin_registerFunctionWithArgCount( "RouterModule_lookup", lookup, ctx, 
# 361 "dht/dhtcore/RouterModule_admin.c" 3 4
   1
# 361 "dht/dhtcore/RouterModule_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "address", .required = 1, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "address", .required = 1, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)


                  ;

    Admin_registerFunctionWithArgCount( "RouterModule_pingNode", pingNode, ctx, 
# 366 "dht/dhtcore/RouterModule_admin.c" 3 4
   1
# 366 "dht/dhtcore/RouterModule_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, })) / sizeof(struct Admin_FunctionArg)), admin)



                  ;

    Admin_registerFunctionWithArgCount( "RouterModule_getPeers", getPeers, ctx, 
# 372 "dht/dhtcore/RouterModule_admin.c" 3 4
   1
# 372 "dht/dhtcore/RouterModule_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, { .name = "nearbyPath", .required = 0, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, { .name = "nearbyPath", .required = 0, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)
    ...
    Admin_registerFunctionWithArgCount( "RouterModule_findNode", findNode, ctx, 
# 379 "dht/dhtcore/RouterModule_admin.c" 3 4
   1
# 379 "dht/dhtcore/RouterModule_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "nodeToQuery", .required = 1, .type = "String" }, { .name = "target", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "nodeToQuery", .required = 1, .type = "String" }, { .name = "target", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" } })) / sizeof(struct Admin_FunctionArg)), admin)
