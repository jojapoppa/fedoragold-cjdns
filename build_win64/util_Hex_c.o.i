...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x037825196697974484;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05230435934683637;
...
struct Linker_x08933932591646221;
...
{
    int outLen = length * 2;
    char* outBuf = Allocator__malloc((alloc),(outLen+1),"Hex.c",100);
    do { if (!((Hex_encode(outBuf, outLen, (uint8_t*) bytes, length) == outLen))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Hex.c"), (101), "(Hex_encode(outBuf, outLen, (uint8_t*) bytes, length) == outLen)"); } } while (0);
    outBuf[outLen] = '\0';
    return outBuf;
}
