...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x003737565371606566;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09484563393252206;
...
struct Linker_x08047999718026915;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x040777023097100695;
...
struct Linker_x05975542085214998;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x04692486127689781;
...
struct Linker_x01934436216486881;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x08390969193636981;
...
struct Linker_x06921129260047671;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x017255483052607;
...
struct Linker_x05146806300523639;
...
struct Linker_x08465806842201096;
...
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x0865857664522879;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x08954404441199724;
...
struct Linker_x0054869654872650386;
...
struct Linker_x022249842369205175;
...
struct Linker_x07553149551965388;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("UpperDistributor_admin.c"), (40), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("UpperDistributor_admin.c"), (61), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull))"); } } while (0); Identity_ptr; }));
    int page = 0;
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("UpperDistributor_admin.c"), (83), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x50850a91365b8c03ull))"); } } while (0); Identity_ptr; }));
    int64_t* udpPort = Dict_getInt(args, (&(String) { .bytes = "udpPort", .len = CString_strlen("udpPort") }));
    if (*udpPort < 1 || *udpPort > 65535) {
                                     ...
                                     struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .alloc = alloc, .ud = ud })),sizeof(*((&(struct Context) { .admin = admin, .alloc = alloc, .ud = ud }))),"UpperDistributor_admin.c",112)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x50850a91365b8c03ull);
