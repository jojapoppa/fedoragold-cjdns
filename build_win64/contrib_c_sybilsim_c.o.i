...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07060773375124769;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09139104025054066;
...
struct Linker_x02632999466304613;
...
struct Linker_x0017200802575860097;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0008330910002255498;
...
struct Linker_x09504761799239427;
...
struct Linker_x05727435847366811;
...
struct Linker_x06417907520463402;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x08137406982398017;
...
# 21 "./io/ArrayReader.h"
struct Linker_x03446889128992905;
# 30 "./io/ArrayReader.h"
...
struct Linker_x016289575419671243;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x0049418952400593685;
...
struct Linker_x008583214237961534;
...
struct Linker_x029675641891495697;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x06180420543692926;
...
struct Linker_x06385001466379083;
...
struct Linker_x08540211737580214;
...
struct Linker_x05522243837859064;
...
struct Linker_x09342124158334679;
...
struct Linker_x0719154870479356;
...
struct Linker_x040705402390814727;
...
struct Linker_x04010203600747835;
...
struct Linker_x0693702765382227;
...
struct Linker_x01896760725613278;
...
struct Linker_x05660784755606352;
...
struct Linker_x05658954007803711;
...
struct Linker_x07246350377071145;
...
struct Linker_x033234278592421784;
...
struct Linker_x028735092970450027;
...
struct Linker_x06520673300050175;
...
struct Linker_x010906740939701232;
...
# 1 "./crypto/Key.h" 1
# 22 "./crypto/Key.h"
struct Linker_x022122576862299148;
...
# 1 "./io/FileReader.h" 1
# 21 "./io/FileReader.h"
struct Linker_x05201724068260314;
# 31 "./io/FileReader.h"
...
# 21 "./io/ArrayWriter.h"
struct Linker_x03109621947028349;
# 30 "./io/ArrayWriter.h"
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x09906577098802343;
# 32 "./util/Hex.h"
...
# 134 "contrib/c/sybilsim.c"
                             )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("sybilsim.c"), (134), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull))"); } } while (0); Identity_ptr; }))
                                     ...
                                     struct FakeNetwork* fakeNet)
{
    ...
    struct NodeContext* node = Allocator__clone((alloc),((&(struct NodeContext) { .alloc = alloc, .base = ctx->base, .nodeLog = { .print = printLog }, .parentLogger = ctx->logger, .nodeName = nodeName })),sizeof(*((&(struct NodeContext) { .alloc = alloc, .base = ctx->base, .nodeLog = { .print = printLog }, .parentLogger = ctx->logger, .nodeName = nodeName }))),"sybilsim.c",191)
       ...
       ;
    (node)->Identity_verifier = ((unsigned long) 0x3c8fe2fda5b8da13ull);
...
static void rpcCallback(struct AdminClient_Promise* promise, struct AdminClient_Result* res)
{
    ...
    (__extension__ ({ __typeof__(ctx) Identity_ptr = (ctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("sybilsim.c"), (328), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull))"); } } while (0); Identity_ptr; }));
...
static void startRpc(void* vcontext)
{
    ...
    (__extension__ ({ __typeof__(ctx) Identity_ptr = (ctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("sybilsim.c"), (342), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3c8fe2fda5b8da13ull))"); } } while (0); Identity_ptr; }));
...
# 370 "contrib/c/sybilsim.c"
    ...
    (ctx)->Identity_verifier = ((unsigned long) 0x3c8fe2fda5b8da13ull);
