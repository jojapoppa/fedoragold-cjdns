...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x07768755312488245;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x008933511645288839;
...
struct Linker_x05514838369386681;
...
# 1 "./benc/Dict.h" 1
# 21 "./benc/Dict.h"
struct Linker_x09208920463014583;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x08508381121989992;
...
struct Linker_x08633533620772189;
...
List* List_new(struct Allocator* alloc)
{
    return Allocator__calloc((alloc),(sizeof(List)),(1),"List.c",24);
}

int32_t List_size(const List* list)
{
    do { if (!((list))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("List.c"), (29), "(list)"); } } while (0);
...
{
    do { if (!((list))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("List.c"), (40), "(list)"); } } while (0);

    struct List_Item* entry = Allocator__malloc((allocator),(sizeof(struct List_Item)),"List.c",42);
    entry->next = *list;
    entry->elem = item;
    *list = entry;
}
...
void List_addInt(List* list, int64_t toAdd, struct Allocator* allocator)
{
    addObject(list, Allocator__clone((allocator),((&(Object) { .type = Object_INTEGER, .as.number = toAdd })),sizeof(*((&(Object) { .type = Object_INTEGER, .as.number = toAdd }))),"List.c",57), allocator);
}


void List_addString(List* list, String* toAdd, struct Allocator* allocator)
{
    addObject(list, Allocator__clone((allocator),((&(Object) { .type = Object_STRING, .as.string = toAdd })),sizeof(*((&(Object) { .type = Object_STRING, .as.string = toAdd }))),"List.c",63), allocator);
}


void List_addDict(List* list, Dict* toAdd, struct Allocator* allocator)
{
    addObject(list, Allocator__clone((allocator),((&(Object) { .type = Object_DICT, .as.dictionary = toAdd })),sizeof(*((&(Object) { .type = Object_DICT, .as.dictionary = toAdd }))),"List.c",69), allocator);
}


void List_addList(List* list, List* toAdd, struct Allocator* allocator)
{
    addObject(list, Allocator__clone((allocator),((&(Object) { .type = Object_LIST, .as.list = toAdd })),sizeof(*((&(Object) { .type = Object_LIST, .as.list = toAdd }))),"List.c",75), allocator);
}

static Object* getObject(const List* list, uint32_t index)
{
    do { if (!((list))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("List.c"), (80), "(list)"); } } while (0);
...
String* List_getString(const List* list, uint32_t index)
{
    do { Object* o = getObject(list, index); if (o && o->type == Object_STRING) { return o->as.string; } return 
# 114 "benc/List.c" 3 4
   ((void *)0)
# 114 "benc/List.c"
   ; } while (0);
...
Dict* List_getDict(const List* list, uint32_t index)
{
    do { Object* o = getObject(list, index); if (o && o->type == Object_DICT) { return o->as.dictionary; } return 
# 120 "benc/List.c" 3 4
   ((void *)0)
# 120 "benc/List.c"
   ; } while (0);
...
List* List_getList(const List* list, uint32_t index)
{
    do { Object* o = getObject(list, index); if (o && o->type == Object_LIST) { return o->as.list; } return 
# 126 "benc/List.c" 3 4
   ((void *)0)
# 126 "benc/List.c"
   ; } while (0);
