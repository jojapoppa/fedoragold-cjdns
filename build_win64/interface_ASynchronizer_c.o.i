...
struct Linker_x03444258635270778;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x05714444346019267;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x03472517558647028;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x040775953210325344;
...
struct Linker_x020615841539508084;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x03652383754157982;
...
struct Linker_x021728041061175163;
...
struct Linker_x03877347000137257;
...
# 1 "./util/Hex.h" 1
# 21 "./util/Hex.h"
struct Linker_x09560292095616059;
...
# 21 "./util/ArrayList.h"
struct Linker_x07907210310371866;
...
{
    struct ASynchronizer_pvt* as = (__extension__ ({ __typeof__((struct ASynchronizer_pvt*) vASynchronizer) Identity_ptr = ((struct ASynchronizer_pvt*) vASynchronizer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ASynchronizer.c"), (50), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull))"); } } while (0); Identity_ptr; }));
...
# 98 "interface/ASynchronizer.c"
                                  )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ASynchronizer.c"), (98), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull))"); } } while (0); Identity_ptr; })); }));
...
# 109 "interface/ASynchronizer.c"
                                  )); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("ASynchronizer.c"), (109), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xf68f57a8c48654b1ull))"); } } while (0); Identity_ptr; })); }));
                                        ...
                                        struct Log* log)
{
    ...
    (ctx)->Identity_verifier = ((unsigned long) 0xf68f57a8c48654b1ull);
    ctx->alloc = alloc;
    ctx->base = base;
    ctx->log = log;
