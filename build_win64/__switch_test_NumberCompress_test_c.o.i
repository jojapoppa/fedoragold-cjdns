...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09540803589384084;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x08709404722663467;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x05358688438775263;
...
struct Linker_x0581171039329303;
...
# 19 "./switch/EncodingScheme.h" 2
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x05592038002105919;
...
struct Linker_x07032329622727085;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x027403347134406064;
...
# 22 "./switch/test/NumberCompress_test.c" 2



# 24 "./switch/test/NumberCompress_test.c"
    ...
    struct EncodingScheme* (* defineScheme)(struct Allocator* alloc) )
{
    ...
    for (uint32_t bits = 0; bits < 64; ++bits) {
        ...
        for (uint32_t i = 0; i < nInterfaces; ++i) {
            ...
            if (1 == i) {
                do { if (!((1 == label))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (53), "(1 == label)"); } } while (0);
                continue;
            }

            do { if (!((bits == bitsUsedForLabel(label)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (57), "(bits == bitsUsedForLabel(label))"); } } while (0);
            do { if (!((i == getDecompressed(label, bits)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (58), "(i == getDecompressed(label, bits))"); } } while (0);
        }
    }

    for (uint64_t label = 0; label < 0x10000u; ++label) {
        uint32_t bits = bitsUsedForLabel(label);
        do { if (!((1 == bitWidths[bits]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (64), "(1 == bitWidths[bits])"); } } while (0);
        if (1 == (label & Bits_maxBits64(bits))) {

            do { if (!((1 == getDecompressed(label, bits)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (67), "(1 == getDecompressed(label, bits))"); } } while (0);
        } else {
            uint32_t i = getDecompressed(label, bits);
            do { if (!((i < nInterfaces))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (70), "(i < nInterfaces)"); } } while (0);
        }
    }

    struct Allocator* alloc = MallocAllocator__new((20000),"NumberCompress_test.c",74);
    ...
    for (uint32_t i = 0; i < nInterfaces; i++) {
        for (int j = 0; j < scheme->count; j++) {
            ...
            for (int k = j; k < scheme->count; k++) {
                ...
                if (1 == i && k != 0) {
                    do { if (!((1 == label))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (84), "(1 == label)"); } } while (0);
                    do { if (!(((~((uint64_t)0)) == labelB))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (85), "((~((uint64_t)0)) == labelB)"); } } while (0);
                    continue;
                }
                int bitsB = bitsUsedForLabel(labelB);
                do { if (!((bitsB == scheme->forms[k].prefixLen + scheme->forms[k].bitCount || (i == 1 && bitsB == 4)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (89), "(bitsB == scheme->forms[k].prefixLen + scheme->forms[k].bitCount || (i == 1 && bitsB == 4))"); } } while (0)
                                              ;

                do { if (!((i == getDecompressed(labelB, bitsB)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (92), "(i == getDecompressed(labelB, bitsB))"); } } while (0);

                uint64_t labelC = EncodingScheme_convertLabel(scheme, labelB, j);
                do { if (!((labelC == label))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("NumberCompress_test.c"), (95), "(labelC == label)"); } } while (0);
            }
        }
    }
    Allocator__free((alloc),"NumberCompress_test.c",99);
}
# 115 "./switch/test/NumberCompress_test.c"
int NumberCompress_test_main(int argc, char** argv);int NumberCompress_test_main(int argc, char** argv)
{
    numberCompressions_generic( 16, NumberCompress_f4_bitsUsedForLabel, NumberCompress_f4_bitsUsedForNumber, NumberCompress_f4_getCompressed, NumberCompress_f4_getDecompressed, NumberCompress_f4_defineScheme );
    numberCompressions_generic( 256, NumberCompress_f8_bitsUsedForLabel, NumberCompress_f8_bitsUsedForNumber, NumberCompress_f8_getCompressed, NumberCompress_f8_getDecompressed, NumberCompress_f8_defineScheme );
    numberCompressions_generic( 257, NumberCompress_v3x5x8_bitsUsedForLabel, NumberCompress_v3x5x8_bitsUsedForNumber, NumberCompress_v3x5x8_getCompressed, NumberCompress_v3x5x8_getDecompressed, NumberCompress_v3x5x8_defineScheme );
    numberCompressions_generic( 256, NumberCompress_v4x8_bitsUsedForLabel, NumberCompress_v4x8_bitsUsedForNumber, NumberCompress_v4x8_getCompressed, NumberCompress_v4x8_getDecompressed, NumberCompress_v4x8_defineScheme );
    return 0;
}
