# 1 "./util/test/QSort_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/test/QSort_test.c"
# 16 "./util/test/QSort_test.c"
# 1 "./util/Assert.h" 1
# 18 "./util/Assert.h"
# 1 "./util/Gcc.h" 1
# 18 "./util/Gcc.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09825569390480182;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
void Assert_failure(const char* format, ...);
# 17 "./util/test/QSort_test.c" 2
# 1 "./util/QSort.h" 1
# 18 "./util/QSort.h"
...
int QSort_test_main(int argc, char** argv);int QSort_test_main(int argc, char** argv)
{
    ...
    for (size_t i = 1; i < sizeof(values) / sizeof(int); i++) {
        do { if (!((last <= values[i]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("QSort_test.c"), (32), "(last <= values[i])"); } } while (0);
        last = values[i];
    }
