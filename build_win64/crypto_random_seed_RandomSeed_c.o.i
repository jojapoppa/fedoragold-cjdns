# 1 "crypto/random/seed/RandomSeed.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "crypto/random/seed/RandomSeed.c"
# 15 "crypto/random/seed/RandomSeed.c"
# 1 "./crypto/random/seed/RandomSeed.h" 1
# 18 "./crypto/random/seed/RandomSeed.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09042561854926965;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x0612622941201554;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 19 "./crypto/random/seed/RandomSeed.h" 2
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x03202550593461877;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 20 "./crypto/random/seed/RandomSeed.h" 2

struct Linker_x09892442286367409;
...
# 16 "crypto/random/seed/RandomSeed.c" 2


# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x08268142748803713;
...
# 19 "crypto/random/seed/RandomSeed.c" 2
...
# 21 "crypto/random/seed/RandomSeed.c" 2
...
void Assert_compileTime(char x[1 - (!(sizeof(struct RandomSeed_Buffer) == 128))]);

static int get(struct RandomSeed* rs, uint64_t buffer[8])
{
    struct RandomSeed_pvt* ctx = (__extension__ ({ __typeof__((struct RandomSeed_pvt*) rs) Identity_ptr = ((struct RandomSeed_pvt*) rs); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa61f132f07e918e3ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RandomSeed.c"), (41), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa61f132f07e918e3ull))"); } } while (0); Identity_ptr; }));
    do { if (ctx->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_INFO, "RandomSeed.c", 42, "Attempting to seed random number generator"); } } while (0);
    ...
    for (int i = 0; i < ctx->rsCount; i++) {
        if (!ctx->rsList[i]->get(ctx->rsList[i], buff.input)) {
            do { if (ctx->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_INFO, "RandomSeed.c", 50, "Trying random seed [%s] Success", ctx->rsList[i]->name); } } while (0);
            crypto_hash_sha512_ref((uint8_t*)buff.output,
                               (uint8_t*)&buff,
                               128);
            successCount++;
        } else {
            do { if (ctx->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_INFO, "RandomSeed.c", 56, "Trying random seed [%s] Failed", ctx->rsList[i]->name); } } while (0);
        }
    }
    do { if (!((sizeof(buff.output) == 64))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RandomSeed.c"), (59), "(sizeof(buff.output) == 64)"); } } while (0);
    Bits__memcpy(buffer,buff.output,64,"RandomSeed.c",60);

    if (successCount > 0) {
        do { if (ctx->logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_INFO, "RandomSeed.c", 63, "Seeding random number generator succeeded with [%d] sources", successCount); } } while (0)
                              ;
        return 0;
    } else {
        do { if (ctx->logger && Log_Level_ERROR >= Log_Level_DEBUG) { Log_print(ctx->logger, Log_Level_ERROR, "RandomSeed.c", 67, "Seeding random number generator failed"); } } while (0);
        return -1;
    }
}
...
struct RandomSeed* RandomSeed_new(RandomSeed_Provider* providers,
                                  ...
                                  struct Allocator* alloc)
{
    struct RandomSeed** rsList = Allocator__calloc((alloc),(sizeof(struct RandomSeed)),(providerCount),"RandomSeed.c",82);
    int i = 0;
    ...
    do { if (!((i > 0))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RandomSeed.c"), (91), "(i > 0)"); } } while (0);

    struct RandomSeed_pvt* out = Allocator__malloc((alloc),(sizeof(struct RandomSeed_pvt)),"RandomSeed.c",93);
    ...
    (out)->Identity_verifier = ((unsigned long) 0xa61f132f07e918e3ull);
