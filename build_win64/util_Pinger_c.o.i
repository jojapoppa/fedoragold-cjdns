...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x06045935899008368;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x003440141091784166;
...
struct Linker_x02882976791049012;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x037612210278290825;
...
struct Linker_x01607208491191745;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x032847375155961567;
...
struct Linker_x09347111388375446;
...
struct Linker_x042514146138170816;
...
struct Linker_x017581369919802503;
...
struct Linker_x05287867323879187;
...
struct Linker_x019170866597931457;
# 50 "./util/Pinger.h"
struct Pinger;
...
struct Linker_x04341778271799661;
...
struct Linker_x06605152446748412;
...
static void callback(String* data, struct Ping* ping)
{
    ...
    Allocator__free((ping->pub.pingAlloc),"Pinger.c",61);
}

static void timeoutCallback(void* vping)
{
    struct Ping* p = (__extension__ ({ __typeof__((struct Ping*) vping) Identity_ptr = ((struct Ping*) vping); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Pinger.c"), (66), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull))"); } } while (0); Identity_ptr; }));
    if (!p->timeSent) {
    ...
    do { if (!((diff < 1000000000))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Pinger.c"), (73), "(diff < 1000000000)"); } } while (0);
    do { if (p->pinger->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(p->pinger->logger, Log_Level_DEBUG, "Pinger.c", 74, "Ping timeout for [%u] in [%lld] ms", p->pub.handle, diff); } } while (0);
    callback(
# 75 "util/Pinger.c" 3 4
            ((void *)0)
# 75 "util/Pinger.c"
                , p);
}
...
{
    struct Ping* p = (__extension__ ({ __typeof__((struct Ping*) job->userData) Identity_ptr = ((struct Ping*) job->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Pinger.c"), (80), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull))"); } } while (0); Identity_ptr; }));
...
{
    struct Ping* p = (__extension__ ({ __typeof__((struct Ping*) vping) Identity_ptr = ((struct Ping*) vping); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Pinger.c"), (95), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull))"); } } while (0); Identity_ptr; }));
                                   ...
                                   struct Pinger* pinger)
{
    ...
    struct Ping* ping = Allocator__clone((alloc),((&(struct Ping) { .pub = { .pingAlloc = alloc, }, .sendPing = sendPing, .pinger = pinger, .timeSent = Time_currentTimeMilliseconds(pinger->eventBase), .onResponse = onResponse })),sizeof(*((&(struct Ping) { .pub = { .pingAlloc = alloc, }, .sendPing = sendPing, .pinger = pinger, .timeSent = Time_currentTimeMilliseconds(pinger->eventBase), .onResponse = onResponse }))),"Pinger.c",109)
       ...
       ;
    (ping)->Identity_verifier = ((unsigned long) 0x59c5c1babb42acadull);
...
# 126 "util/Pinger.c" 3 4
                                     ((void *)0)
# 126 "util/Pinger.c"
                                         , ((data) ? data->len : 0) + 12, alloc);
    Bits__memcpy(toSend->bytes,&ping->pub.handle,4,"Pinger.c",127);
    Bits__memcpy(&toSend->bytes[4],&ping->cookie,8,"Pinger.c",128);
    if (data) {
        Bits__memcpy(toSend->bytes + 12,data->bytes,data->len,"Pinger.c",130);
    }
    ping->data = toSend;

    Allocator__onFree((alloc), (freePing), (ping), "Pinger.c", 134);

    ping->timeout =
        Timeout__setTimeout((timeoutCallback), (ping), (timeoutMilliseconds), (pinger->eventBase), (alloc), "Pinger.c", 137);

    Timeout__setTimeout((asyncSendPing), (ping), (0), (pinger->eventBase), (alloc), "Pinger.c", 139);
...
{
    if (data->len < 12) {
        do { if (pinger->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(pinger->logger, Log_Level_DEBUG, "Pinger.c", 147, "Invalid ping response, too short"); } } while (0);
        return;
    }
    uint32_t handle;
    Bits__memcpy(&handle,data->bytes,4,"Pinger.c",151);
    int index = Map_OutstandingPings_indexForHandle(handle - pinger->baseHandle,
                                                    &pinger->outstandingPings);
    if (index < 0) {
        do { if (pinger->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(pinger->logger, Log_Level_DEBUG, "Pinger.c", 155, "Invalid ping response handle [%u].", handle); } } while (0);
    } else {
        data->len -= 4;
        data->bytes += 4;
        uint64_t cookie;
        Bits__memcpy(&cookie,data->bytes,8,"Pinger.c",160);
        struct Ping* p = (__extension__ ({ __typeof__((struct Ping*) pinger->outstandingPings.values[index]) Identity_ptr = ((struct Ping*) pinger->outstandingPings.values[index]); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Pinger.c"), (161), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x59c5c1babb42acadull))"); } } while (0); Identity_ptr; }));
        if (cookie != p->cookie) {
            do { if (pinger->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(pinger->logger, Log_Level_DEBUG, "Pinger.c", 163, "Ping response with invalid cookie"); } } while (0);
            return;
        }
        data->len -= 8;
        data->bytes += 8;
        if (!p->timeSent) {
            do { if (pinger->logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(pinger->logger, Log_Level_DEBUG, "Pinger.c", 169, "Duplicate response"); } } while (0);
            return;
        }
        callback(data, p);
    }
...
struct Pinger* Pinger_new(struct EventBase* eventBase,
                          ...
                          struct Allocator* alloc)
{
    struct Pinger* out = Allocator__clone((alloc),((&(struct Pinger) { .outstandingPings = { .allocator = alloc }, .rand = rand, .eventBase = eventBase, .logger = logger, .allocator = alloc })),sizeof(*((&(struct Pinger) { .outstandingPings = { .allocator = alloc }, .rand = rand, .eventBase = eventBase, .logger = logger, .allocator = alloc }))),"Pinger.c",181)
