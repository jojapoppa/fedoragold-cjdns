# 1 "dht/dhtcore/Janitor_admin.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "dht/dhtcore/Janitor_admin.c"
# 15 "dht/dhtcore/Janitor_admin.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x045475950072131655;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x008041821001040206;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 16 "dht/dhtcore/Janitor_admin.c" 2
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
# 20 "./exception/Except.h"
struct Linker_x05357662358761301;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x05613475901351206;
...
struct Linker_x0567923482670579;
...
struct Linker_x044091113751848554;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x0366521420378048;
...
struct Linker_x003102218262544909;
...
struct Linker_x05899959893717035;
...
struct Linker_x01401520730000263;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x02306317229496173;
...
struct Linker_x05377062031879167;
...
struct Linker_x05808102194646279;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x02312740304923897;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x05404002830041417;
...
struct Linker_x001137248602267893;
...
struct Linker_x07394572161465038;
...
struct Linker_x08493705259663376;
...
struct Linker_x05170412909088946;
...
struct Linker_x07936710024135929;
...
struct Linker_x06944549721121849;
...
struct Linker_x0045877080560923655;
...
struct Janitor* Janitor_new(struct RouterModule* routerModule,
                            ...
                            struct EventBase* eventBase,
                            struct Random* rand);
# 18 "dht/dhtcore/Janitor_admin.c" 2
...
{
    if (String_equals((&(String) { .bytes = "externalMill", .len = CString_strlen("externalMill") }), name)) {
        return ctx->janitor->externalMill;
    } else if (String_equals((&(String) { .bytes = "linkMill", .len = CString_strlen("linkMill") }), name)) {
        return ctx->janitor->linkMill;
    } else if (String_equals((&(String) { .bytes = "nodeMill", .len = CString_strlen("nodeMill") }), name)) {
        return ctx->janitor->nodeMill;
    } else if (String_equals((&(String) { .bytes = "dhtMill", .len = CString_strlen("dhtMill") }), name)) {
        return ctx->janitor->dhtMill;
    } else if (String_equals((&(String) { .bytes = "splitMill", .len = CString_strlen("splitMill") }), name)) {
        return ctx->janitor->splitMill;
    } else {
        return 
# 40 "dht/dhtcore/Janitor_admin.c" 3 4
              ((void *)0)
# 40 "dht/dhtcore/Janitor_admin.c"
                  ;
    }
}
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x1a3e5c3c1bd0ab75ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Janitor_admin.c"), (47), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x1a3e5c3c1bd0ab75ull))"); } } while (0); Identity_ptr; }));

    Dict* out = Dict_new(requestAlloc);
    struct RumorMill* rm = getRumorMill(ctx, Dict_getString(args, (&(String) { .bytes = "mill", .len = CString_strlen("mill") })));
    if (!rm) {
        Dict_putString(out, String_new("error", requestAlloc), (&(String) { .bytes = "mill must be one of " "[externalMill,linkMill,nodeMill,dhtMill,splitMill]", .len = CString_strlen("mill must be one of " "[externalMill,linkMill,nodeMill,dhtMill,splitMill]") }), requestAlloc)
    ...
    for (int i = 0; i < 4 && ctr < rm->count; i++) {
        String* addr = Address_toString(&rm->addresses[ctr++], requestAlloc);
        List_addString(table, addr, requestAlloc);
    }
    Dict_putList(out, String_new("addresses", requestAlloc), table, requestAlloc);
    Dict_putInt(out, String_new("total", requestAlloc), rm->count, requestAlloc);
    Admin_sendMessage(out, txid, ctx->admin);
}

void Janitor_admin_register(struct Janitor* janitor, struct Admin* admin, struct Allocator* alloc)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .alloc = alloc, .janitor = janitor })),sizeof(*((&(struct Context) { .admin = admin, .alloc = alloc, .janitor = janitor }))),"Janitor_admin.c",76)



       ;
    (ctx)->Identity_verifier = ((unsigned long) 0x1a3e5c3c1bd0ab75ull);

    Admin_registerFunctionWithArgCount( "Janitor_dumpRumorMill", dumpRumorMill, ctx, 
# 83 "dht/dhtcore/Janitor_admin.c" 3 4
   1
# 83 "dht/dhtcore/Janitor_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "page", .required = 1, .type = "Int" }, { .name = "mill", .required = 1, .type = "String" }, }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "page", .required = 1, .type = "Int" }, { .name = "mill", .required = 1, .type = "String" }, })) / sizeof(struct Admin_FunctionArg)), admin)
