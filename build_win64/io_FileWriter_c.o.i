...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x027409629802581437;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x009465523830354217;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 20 "./io/FileWriter.h" 2

struct Linker_x07648495350923952;
...
{
    struct FileWriter_context* context = (__extension__ ({ __typeof__((struct FileWriter_context*) writer) Identity_ptr = ((struct FileWriter_context*) writer); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x3164eb2209acd8fdull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileWriter.c"), (28), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x3164eb2209acd8fdull))"); } } while (0); Identity_ptr; }));
...
struct Writer* FileWriter_new(FILE* writeTo, struct Allocator* allocator)
{
    struct FileWriter_context* context = Allocator__clone((allocator),((&(struct FileWriter_context) { .generic = { .write = write }, .writeTo = writeTo })),sizeof(*((&(struct FileWriter_context) { .generic = { .write = write }, .writeTo = writeTo }))),"FileWriter.c",41)




       ;
    (context)->Identity_verifier = ((unsigned long) 0x3164eb2209acd8fdull);
