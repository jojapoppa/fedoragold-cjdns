# 1 "util/events/libuv/FileNo_admin.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "util/events/libuv/FileNo_admin.c"
# 15 "util/events/libuv/FileNo_admin.c"
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
struct Linker_x08214701527589705;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x010597641792620327;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x02007701280276175;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x024832704253813764;
...
struct Linker_x04888225273953539;
...
struct Linker_x005984702028083522;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x07210888562964952;
...
struct Linker_x07241382524275009;
...
struct Linker_x05736080904348102;
...
struct Admin* Admin_new(struct AddrIface* ai,
                        ...
                        String* password);
# 16 "util/events/libuv/FileNo_admin.c" 2

# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x06394227840849476;
...
# 18 "util/events/libuv/FileNo_admin.c" 2
# 1 "./benc/String.h" 1
# 20 "./benc/String.h"
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x0571467338236086;
...
struct Linker_x09761334617517103;
...
# 19 "util/events/libuv/FileNo_admin.c" 2


# 1 "./util/events/FileNo.h" 1
# 23 "./util/events/FileNo.h"
struct Linker_x023028267259602098;
...
struct FileNo_Promise* FileNo_import(const char* path,
                                     ...
                                     struct Log* logger,
                                     struct Allocator* alloc);
# 22 "util/events/libuv/FileNo_admin.c" 2
# 1 "./util/events/libuv/FileNo_admin.h" 1
# 18 "./util/events/libuv/FileNo_admin.h"
# 1 "./admin/Admin.h" 1
# 19 "./util/events/libuv/FileNo_admin.h" 2





struct Linker_x09566346046875751;
...
void FileNo_admin_register(struct Admin* admin,
                           ...
                           struct Log* logger,
                           struct Except* eh);
# 23 "util/events/libuv/FileNo_admin.c" 2
# 1 "./util/events/Time.h" 1
# 20 "./util/events/Time.h"
struct Linker_x022119444854392123;
...
# 24 "util/events/libuv/FileNo_admin.c" 2
# 1 "./util/events/Timeout.h" 1
# 20 "./util/events/Timeout.h"
struct Linker_x033512263762686545;
...
# 25 "util/events/libuv/FileNo_admin.c" 2
...
{
    struct FileNoRequest* fr= (__extension__ ({ __typeof__((struct FileNoRequest*)promise->userData) Identity_ptr = ((struct FileNoRequest*)promise->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa5adf8d29ed5a7eeull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileNo_admin.c"), (49), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa5adf8d29ed5a7eeull))"); } } while (0); Identity_ptr; }));
    Dict* resp = Dict_new(promise->alloc);

    Dict_putInt(resp, String_new("tunfd", promise->alloc), tunfd, promise->alloc);
    Dict_putInt(resp, String_new("type", promise->alloc), fr->type, promise->alloc);
    Dict_putString(resp, String_new("error", promise->alloc), String_new("none", promise->alloc), promise->alloc);
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xa5adf8d29ed5a7eeull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("FileNo_admin.c"), (60), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xa5adf8d29ed5a7eeull))"); } } while (0); Identity_ptr; }));
    String* path = Dict_getString(args, (&(String) { .bytes = "path", .len = CString_strlen("path") }));
    String* type = Dict_getString(args, (&(String) { .bytes = "type", .len = CString_strlen("type") }));
    char* err = 
# 63 "util/events/libuv/FileNo_admin.c" 3 4
               ((void *)0)
# 63 "util/events/libuv/FileNo_admin.c"
                   ;
...
# 67 "util/events/libuv/FileNo_admin.c" 3 4
                      ((void *)0) 
# 67 "util/events/libuv/FileNo_admin.c"
                      });
    ...
    struct FileNo_Promise* fp = FileNo_import(path->bytes, ctx->base, ctx->eh, ctx->logger,
                                              ctx->alloc);
    struct FileNoRequest* fr = Allocator__calloc((fp->alloc),(sizeof(struct FileNoRequest)),(1),"FileNo_admin.c",79);
    (fr)->Identity_verifier = ((unsigned long) 0xa5adf8d29ed5a7eeull);
    fr->txid = ((txid) ? String_newBinary(txid->bytes, txid->len, fp->alloc) : 
# 81 "util/events/libuv/FileNo_admin.c" 3 4
              ((void *)0)
# 81 "util/events/libuv/FileNo_admin.c"
              );
    fr->fp = fp;
    fr->type = fdtype;
    fr->ctx = ctx;
...
void FileNo_admin_register(struct Admin* admin,
                           ...
                           struct Except* eh)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .admin = admin, .eh = eh, .alloc = alloc, .logger = logger, .base = base })),sizeof(*((&(struct Context) { .admin = admin, .eh = eh, .alloc = alloc, .logger = logger, .base = base }))),"FileNo_admin.c",96)
       ...
       ;
    (ctx)->Identity_verifier = ((unsigned long) 0xa5adf8d29ed5a7eeull);

    Admin_registerFunctionWithArgCount( "FileNo_import", import, ctx, 
# 105 "util/events/libuv/FileNo_admin.c" 3 4
   1
# 105 "util/events/libuv/FileNo_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "type", .required = 0, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "type", .required = 0, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)
