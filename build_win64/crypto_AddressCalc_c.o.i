# 1 "crypto/AddressCalc.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "crypto/AddressCalc.c"
# 15 "crypto/AddressCalc.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x04689667267695363;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09925164492255527;
...
# 17 "crypto/AddressCalc.c" 2
# 1 "./util/Endian.h" 1
# 18 "crypto/AddressCalc.c" 2
# 1 "./crypto/AddressCalc.h" 1
# 19 "./crypto/AddressCalc.h"
struct Linker_x08139146202956955;
...
# 19 "crypto/AddressCalc.c" 2
# 31 "crypto/AddressCalc.c"
...
# 32 "crypto/AddressCalc.c" 2

_Bool 
# 33 "crypto/AddressCalc.c"
    AddressCalc_validAddress(const uint8_t address[16])
{
    uint64_t significant_bits;
    Bits__memcpy(&significant_bits,address,sizeof(uint64_t),"AddressCalc.c",36);
    return (significant_bits & (((uint64_t)__builtin_bswap64(((uint64_t)(
# 37 "crypto/AddressCalc.c" 3 4
                              0xffffffffffffffffULL 
# 37 "crypto/AddressCalc.c"
                              << (64 - 8))))))) == (((uint64_t)__builtin_bswap64(((uint64_t)(((uint64_t) 0xfc) << (64 - 8))))));
}

void AddressCalc_makeValidAddress(uint8_t address[16])
{
    uint64_t significant_bits;
    Bits__memcpy(&significant_bits,address,sizeof(uint64_t),"AddressCalc.c",43);
    significant_bits &= ~(((uint64_t)__builtin_bswap64(((uint64_t)(
# 44 "crypto/AddressCalc.c" 3 4
                        0xffffffffffffffffULL 
# 44 "crypto/AddressCalc.c"
                        << (64 - 8))))));
    significant_bits |= (((uint64_t)__builtin_bswap64(((uint64_t)(((uint64_t) 0xfc) << (64 - 8))))));
    Bits__memcpy(address,&significant_bits,sizeof(uint64_t),"AddressCalc.c",46);
}


# 49 "crypto/AddressCalc.c" 3 4
_Bool 
# 49 "crypto/AddressCalc.c"
    AddressCalc_addressForPublicKey(uint8_t addressOut[16], const uint8_t key[32])
{
    uint8_t hash[64];
    crypto_hash_sha512_ref(hash, key, 32);
    crypto_hash_sha512_ref(hash, hash, 64);
    if (addressOut) {
        Bits__memcpy(addressOut,hash,16,"AddressCalc.c",55);
    }
