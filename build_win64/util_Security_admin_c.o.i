# 1 "util/Security_admin.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "util/Security_admin.c"
# 15 "util/Security_admin.c"
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
struct Linker_x02494252075050527;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x03666777830130157;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x03638345842327406;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x036465312660486005;
...
struct Linker_x007146333951877448;
...
struct Linker_x08889220595905376;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x0537041874609036;
...
struct Linker_x02676175385782722;
...
struct Linker_x0502577992503261;
...
struct Admin* Admin_new(struct AddrIface* ai,
                        ...
                        String* password);
# 16 "util/Security_admin.c" 2
# 1 "./benc/String.h" 1
# 20 "./benc/String.h"
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x08282499811026831;
...
struct Linker_x05328737827221302;
...
# 20 "util/Security_admin.c" 2

# 1 "./util/Security.h" 1
# 27 "./util/Security.h"
    struct Linker_x08581340205699786;
...
# 22 "util/Security_admin.c" 2
# 1 "./util/Security_admin.h" 1
# 18 "./util/Security_admin.h"
# 1 "./admin/Admin.h" 1
# 19 "./util/Security_admin.h" 2




struct Linker_x09030640673071502;

void Security_admin_register(struct Allocator* alloc,
                             struct Log* logger,
                             struct Security* sec,
                             struct Admin* admin);
# 23 "util/Security_admin.c" 2
...
{
    Dict error = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = (&(String) { .bytes = errorMessage, .len = CString_strlen(errorMessage) }) }), .next = 
# 34 "util/Security_admin.c" 3 4
                ((void *)0) 
# 34 "util/Security_admin.c"
                });
...
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (40), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
    struct Jmp jmp;
    jmp.handler.exception = Jmp_callback; if (!
# 42 "util/Security_admin.c" 3
   _setjmp((
# 42 "util/Security_admin.c"
   jmp.buf
# 42 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 42 "util/Security_admin.c"
   ) {
        int64_t* user = Dict_getInt(args, (&(String) { .bytes = "uid", .len = CString_strlen("uid") }));
        int64_t* group = Dict_getInt(args, (&(String) { .bytes = "gid", .len = CString_strlen("gid") }));
        int gid = group ? (int)*group : 0;
        int64_t* keepNetAdmin = Dict_getInt(args, (&(String) { .bytes = "keepNetAdmin", .len = CString_strlen("keepNetAdmin") }));
                              ...
                              struct Allocator* requestAlloc)
{
    ...
    Dict_putInt(out, String_new("noOpenFiles", requestAlloc), sp->noOpenFiles, requestAlloc);
    Dict_putInt(out, String_new("seccompExists", requestAlloc), sp->seccompExists, requestAlloc);
    Dict_putInt(out, String_new("seccompEnforcing", requestAlloc), sp->seccompEnforcing, requestAlloc);
    Dict_putInt(out, String_new("userId", requestAlloc), sp->uid, requestAlloc);
...
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (72), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
    struct Jmp jmp;
    jmp.handler.exception = Jmp_callback; if (!
# 74 "util/Security_admin.c" 3
   _setjmp((
# 74 "util/Security_admin.c"
   jmp.buf
# 74 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 74 "util/Security_admin.c"
...
# 96 "util/Security_admin.c"
static void nofiles(Dict* args, void* vctx, String* txid, struct Allocator* requestAlloc)
{
    do { struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (98), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; })); struct Jmp jmp; jmp.handler.exception = Jmp_callback; if (!
# 98 "util/Security_admin.c" 3
   _setjmp((
# 98 "util/Security_admin.c"
   jmp.buf
# 98 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 98 "util/Security_admin.c"
   ) { Security_nofiles(&jmp.handler); } else { sendError(jmp.message, txid, ctx->admin); return; } sendError("none", txid, ctx->admin); } while (0);
}

static void noforks(Dict* args, void* vctx, String* txid, struct Allocator* requestAlloc)
{
    do { struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (103), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; })); struct Jmp jmp; jmp.handler.exception = Jmp_callback; if (!
# 103 "util/Security_admin.c" 3
   _setjmp((
# 103 "util/Security_admin.c"
   jmp.buf
# 103 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 103 "util/Security_admin.c"
   ) { Security_noforks(&jmp.handler); } else { sendError(jmp.message, txid, ctx->admin); return; } sendError("none", txid, ctx->admin); } while (0);
}

static void chroot(Dict* args, void* vctx, String* txid, struct Allocator* requestAlloc)
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (108), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
    struct Jmp jmp;
    jmp.handler.exception = Jmp_callback; if (!
# 110 "util/Security_admin.c" 3
   _setjmp((
# 110 "util/Security_admin.c"
   jmp.buf
# 110 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 110 "util/Security_admin.c"
   ) {
        String* root = Dict_getString(args, (&(String) { .bytes = "root", .len = CString_strlen("root") }));
...
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (122), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
    struct Jmp jmp;
    struct Except* eh = &jmp.handler;
    jmp.handler.exception = Jmp_callback; if (!
# 125 "util/Security_admin.c" 3
   _setjmp((
# 125 "util/Security_admin.c"
   jmp.buf
# 125 "util/Security_admin.c" 3
   ), __builtin_frame_address (0))
# 125 "util/Security_admin.c"
...
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (141), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* const ctx = (__extension__ ({ __typeof__((struct Context*) vctx) Identity_ptr = ((struct Context*) vctx); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Security_admin.c"), (148), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xea8a53dae1c188ecull))"); } } while (0); Identity_ptr; }));
    String* user = Dict_getString(args, (&(String) { .bytes = "user", .len = CString_strlen("user") }));
    Dict* ret = Security_getUser((user) ? user->bytes : 
# 150 "util/Security_admin.c" 3 4
                                                       ((void *)0)
# 150 "util/Security_admin.c"
                                                           , requestAlloc);
...
void Security_admin_register(struct Allocator* alloc,
                             ...
                             struct Admin* admin)
{
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .logger = logger, .admin = admin })),sizeof(*((&(struct Context) { .logger = logger, .admin = admin }))),"Security_admin.c",159)


       ;
    (ctx)->Identity_verifier = ((unsigned long) 0xea8a53dae1c188ecull);
    ctx->sec = sec;

    Admin_registerFunctionWithArgCount( "Security_nofiles", nofiles, ctx, 
# 166 "util/Security_admin.c" 3 4
   1
# 166 "util/Security_admin.c"
   , 
# 166 "util/Security_admin.c" 3 4
   ((void *)0)
# 166 "util/Security_admin.c"
...
# 166 "util/Security_admin.c"
    ...
    Admin_registerFunctionWithArgCount( "Security_noforks", noforks, ctx, 
# 167 "util/Security_admin.c" 3 4
   1
# 167 "util/Security_admin.c"
   , 
# 167 "util/Security_admin.c" 3 4
   ((void *)0)
# 167 "util/Security_admin.c"
...
# 167 "util/Security_admin.c"
    ...
    Admin_registerFunctionWithArgCount( "Security_chroot", chroot, ctx, 
# 168 "util/Security_admin.c" 3 4
   1
# 168 "util/Security_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "root", .required = 1, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "root", .required = 1, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)

              ;
    Admin_registerFunctionWithArgCount( "Security_setUser", setUser, ctx, 
# 171 "util/Security_admin.c" 3 4
   1
# 171 "util/Security_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "uid", .required = 1, .type = "Int" }, { .name = "gid", .required = 0, .type = "Int" }, { .name = "keepNetAdmin", .required = 1, .type = "Int" }, }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "uid", .required = 1, .type = "Int" }, { .name = "gid", .required = 0, .type = "Int" }, { .name = "keepNetAdmin", .required = 1, .type = "Int" }, })) / sizeof(struct Admin_FunctionArg)), admin)



              ;
    Admin_registerFunctionWithArgCount( "Security_getUser", getUser, ctx, 
# 176 "util/Security_admin.c" 3 4
   1
# 176 "util/Security_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "user", .required = 0, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "user", .required = 0, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)

              ;
    Admin_registerFunctionWithArgCount( "Security_seccomp", seccomp, ctx, 
# 179 "util/Security_admin.c" 3 4
   1
# 179 "util/Security_admin.c"
   , 
# 179 "util/Security_admin.c" 3 4
   ((void *)0)
# 179 "util/Security_admin.c"
...
# 179 "util/Security_admin.c"
    ...
    Admin_registerFunctionWithArgCount( "Security_setupComplete", setupComplete, ctx, 
# 180 "util/Security_admin.c" 3 4
   1
# 180 "util/Security_admin.c"
   , 
# 180 "util/Security_admin.c" 3 4
   ((void *)0)
# 180 "util/Security_admin.c"
...
# 180 "util/Security_admin.c"
    ...
    Admin_registerFunctionWithArgCount( "Security_checkPermissions", checkPermissions, ctx, 
# 181 "util/Security_admin.c" 3 4
   1
# 181 "util/Security_admin.c"
   , 
# 181 "util/Security_admin.c" 3 4
   ((void *)0)
# 181 "util/Security_admin.c"
