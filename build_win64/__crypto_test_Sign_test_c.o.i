# 1 "./crypto/test/Sign_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./crypto/test/Sign_test.c"
# 15 "./crypto/test/Sign_test.c"
# 1 "./crypto/Sign.h" 1
# 18 "./crypto/Sign.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x0016531568414349662;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x049212273490283276;
...
struct Linker_x06688895013732079;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x014031386703549908;
...
struct Linker_x07401587681119894;
...
struct Linker_x04591469302661422;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x06412919988389476;
...
struct Linker_x05589884987275813;
...
# 23 "./crypto/test/Sign_test.c" 2

int Sign_test_main(int argc, char** argv);int Sign_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((1048576),"Sign_test.c",26);
    struct Log* logger = FileWriterLog_new(
# 27 "./crypto/test/Sign_test.c" 3
                                          (__acrt_iob_func(1))
# 27 "./crypto/test/Sign_test.c"
                                                , alloc);
    struct Random* rand = Random_new(alloc, logger, 
# 28 "./crypto/test/Sign_test.c" 3 4
                                                   ((void *)0)
# 28 "./crypto/test/Sign_test.c"
                                                       );

    uint8_t curve25519private[32];
    Random_bytes(rand, curve25519private, 32);
    uint8_t curve25519public[32];
    crypto_scalarmult_curve25519_donna_c64_base(curve25519public, curve25519private);
...
# 38 "./crypto/test/Sign_test.c" 3 4
                                        ((void *)0)
# 38 "./crypto/test/Sign_test.c"
                                            );
    Sign_signMsg(signingKeyPair, msg, rand);

    uint8_t curve25519publicB[32];
    do { if (!((!Sign_verifyMsg(&signingKeyPair[32], msg)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_test.c"), (42), "(!Sign_verifyMsg(&signingKeyPair[32], msg))"); } } while (0);
    do { if (!((!Sign_publicSigningKeyToCurve25519(curve25519publicB, &signingKeyPair[32])))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_test.c"), (43), "(!Sign_publicSigningKeyToCurve25519(curve25519publicB, &signingKeyPair[32]))"); } } while (0);
    do { if (!((!__builtin_memcmp(curve25519publicB,curve25519public,32)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sign_test.c"), (44), "(!__builtin_memcmp(curve25519publicB,curve25519public,32))"); } } while (0);

    Allocator__free((alloc),"Sign_test.c",46);
    return 0;
}
