...
struct Linker_x07580535825868153;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x08981995886253633;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x03226215913370991;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x09366272049146684;
...
struct Linker_x04336574427187383;
...
struct Linker_x009154822084429193;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06793322596519309;
...
struct Linker_x0205482113199847;
...
struct Linker_x04787465324769373;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x08418913432988193;
...
struct Linker_x07081902482765143;
...
# 21 "./util/ArrayList.h"
struct Linker_x07723739786355159;
...
struct Linker_x019474505034378997;
...
struct Linker_x06939064070684124;
...
struct Linker_x015179523183234944;
...
struct Linker_x047863555300460914;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x021900822180229684;
...
# 19 "./util/AddrTools.h"
struct Linker_x08464736756826627;
...
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AuthorizedPasswords.c"), (38), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull))"); } } while (0); Identity_ptr; }));
...
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AuthorizedPasswords.c"), (71), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull))"); } } while (0); Identity_ptr; }));
    String* user = Dict_getString(args, (&(String) { .bytes = "user", .len = CString_strlen("user") }));
...
{
    struct Context* context = (__extension__ ({ __typeof__((struct Context*) vcontext) Identity_ptr = ((struct Context*) vcontext); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("AuthorizedPasswords.c"), (84), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x60a7719b26d929f9ull))"); } } while (0); Identity_ptr; }));
                              ...
                              struct Allocator* allocator)
{
    ...
    context->ca = ca;
    (context)->Identity_verifier = ((unsigned long) 0x60a7719b26d929f9ull);
...
# 110 "admin/AuthorizedPasswords.c"
   , ((struct Admin_FunctionArg[]){ { .name = "password", .required = 1, .type = "String" }, { .name = "ipv6", .required = 0, .type = "String" }, { .name = "user", .required = 0, .type = "String" } }), (sizeof(((struct Admin_FunctionArg[]){ { .name = "password", .required = 1, .type = "String" }, { .name = "ipv6", .required = 0, .type = "String" }, { .name = "user", .required = 0, .type = "String" } })) / sizeof(struct Admin_FunctionArg)), admin)
                  ...
                  ;
    Admin_registerFunctionWithArgCount( "AuthorizedPasswords_remove", removeAuth, context, 
