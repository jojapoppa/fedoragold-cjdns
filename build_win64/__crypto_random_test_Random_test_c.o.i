# 1 "./crypto/random/test/Random_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./crypto/random/test/Random_test.c"
# 15 "./crypto/random/test/Random_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x0012785528657417;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x024513956606807774;
...
struct Linker_x042245874526459426;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x041111830160425367;
...
struct Linker_x09590619764881227;
...
struct Linker_x041686941149907053;
...
struct Linker_x06617641450899054;
...
# 22 "./crypto/random/test/Random_test.c" 2

# 1 "./util/log/WriterLog.h" 1
# 22 "./util/log/WriterLog.h"
struct Linker_x046522053592971346;

struct Log* WriterLog_new(struct Writer* w, struct Allocator* alloc);
# 24 "./crypto/random/test/Random_test.c" 2

static void checkBytes(struct Random* rand, int alignment, int length)
{
    do { if (!((length < 128 && alignment < 8))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (27), "(length < 128 && alignment < 8)"); } } while (0);
    ...
    Bits__memcpy(oldBuff,buff,length,"Random_test.c",38);

    uint8_t sameAsOld[128];
    __builtin_memset(sameAsOld,0xff,128);



    uint8_t sameBytes[128+128];
    __builtin_memset(sameBytes,0xff,128+128);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        do { if (!((Bits_isZero(origBuff, 8+alignment)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (63), "(Bits_isZero(origBuff, 8+alignment))"); } } while (0);
        do { if (!((Bits_isZero(buff+length, 8)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (64), "(Bits_isZero(buff+length, 8))"); } } while (0);
    }

    for (int i = 0; i < length+length-1; i++) {
        do { if (!((!sameBytes[i]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (68), "(!sameBytes[i])"); } } while (0);
    }
    for (int i = 0; i < length; i++) {
        do { if (!((!sameAsOld[i]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (71), "(!sameAsOld[i])"); } } while (0);
    }
}
...
# 81 "./crypto/random/test/Random_test.c" 3 4
                                                   ((void *)0)
# 81 "./crypto/random/test/Random_test.c"
                                                       );
    struct Random* rand2 = Random_new(alloc, logger, 
# 82 "./crypto/random/test/Random_test.c" 3 4
                                                    ((void *)0)
# 82 "./crypto/random/test/Random_test.c"
                                                        );

    Random_bytes(rand, buff, 32);
    Random_bytes(rand2, buff, 32);

    do { if (!((__builtin_memcmp(buff,buff2,32)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random_test.c"), (87), "(__builtin_memcmp(buff,buff2,32))"); } } while (0);
}

int Random_test_main(int argc, char** argv);int Random_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((1<<20),"Random_test.c",92);
    struct RandomSeed* seed = DeterminentRandomSeed_new(alloc, 
# 93 "./crypto/random/test/Random_test.c" 3 4
                                                              ((void *)0)
# 93 "./crypto/random/test/Random_test.c"
                                                                  );
    struct Writer* w = FileWriter_new(
# 94 "./crypto/random/test/Random_test.c" 3
                                     (__acrt_iob_func(1))
# 94 "./crypto/random/test/Random_test.c"
                                           , alloc);
    struct Log* logger = WriterLog_new(w, alloc);
    struct Random* rand = Random_newWithSeed(alloc, logger, seed, 
# 96 "./crypto/random/test/Random_test.c" 3 4
                                                                 ((void *)0)
# 96 "./crypto/random/test/Random_test.c"
                                                                     );
    ...
    checkBytes(rand, 3, 29);
    checkBytes(rand, 6, 10);
    checkBytes(rand, 4, 20);


    checkBytes(rand, 0, 9);
    checkBytes(rand, 0, 14);
    checkBytes(rand, 0, 20);


    checkBytes(rand, 1, 28);
    checkBytes(rand, 3, 19);
    checkBytes(rand, 5, 23);
    checkBytes(rand, 2, 9);
    checkBytes(rand, 6, 12);
    checkBytes(rand, 6, 14);
    checkBytes(rand, 4, 19);
    checkBytes(rand, 4, 10);
    checkBytes(rand, 4, 32);


    test179(alloc, logger);
# 134 "./crypto/random/test/Random_test.c"
    Allocator__free((alloc),"Random_test.c",134);
    return 0;
}
