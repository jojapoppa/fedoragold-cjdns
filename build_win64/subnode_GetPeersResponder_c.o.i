# 1 "subnode/GetPeersResponder.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "subnode/GetPeersResponder.c"
# 15 "subnode/GetPeersResponder.c"
# 1 "./subnode/GetPeersResponder.h" 1
# 18 "./subnode/GetPeersResponder.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x08955522846746053;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x04547364958814375;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 19 "./subnode/GetPeersResponder.h" 2
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x03284150694818162;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 20 "./subnode/GetPeersResponder.h" 2
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x09074807282669817;
...
struct Linker_x054701745153913;
...
struct Linker_x045058153102924425;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x0675659305127571;
...
# 21 "./subnode/GetPeersResponder.h" 2
...
struct Linker_x08821034260822511;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x07834337536395366;
...
struct Linker_x06552496848455474;
...
struct Linker_x04501245557434914;
...
struct Linker_x07386674807019538;
...
# 1 "./benc/Dict.h" 1
# 21 "./benc/Dict.h"
struct Linker_x08141490860661027;
...
# 1 "./benc/List.h" 1
# 21 "./benc/List.h"
struct Linker_x011331368844856615;
...
struct Linker_x04737497986014987;
...
struct Linker_x024674822662679952;
...
struct BoilerplateResponder* BoilerplateResponder_new(struct EncodingScheme* myScheme,
                                                      struct Allocator* alloc);
# 23 "./subnode/GetPeersResponder.h" 2



struct Linker_x0051902353500774545;
...
void ReplySerializer_serialize(struct Address_List* addrs,
                               ...
                               struct Allocator* alloc);
# 18 "subnode/GetPeersResponder.c" 2
                       ...
                       struct MsgCore_Handler* handler)
{
    struct GetPeersResponder_pvt* gprp =
        (__extension__ ({ __typeof__((struct GetPeersResponder_pvt*) handler->userData) Identity_ptr = ((struct GetPeersResponder_pvt*) handler->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xd8d0ba082c9d9665ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("GetPeersResponder.c"), (42), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xd8d0ba082c9d9665ull))"); } } while (0); Identity_ptr; }));
    do { if (gprp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(gprp->log, Log_Level_DEBUG, "GetPeersResponder.c", 43, "Received getPeers req from [%s]", Address_toString(src, tmpAlloc)->bytes); } } while (0);

    String* txid = Dict_getString(msg, (&(String) { .bytes = "txid", .len = CString_strlen("txid") }));
    if (!txid) {
        do { if (gprp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(gprp->log, Log_Level_DEBUG, "GetPeersResponder.c", 47, "getPeers missing txid"); } } while (0);
        return;
    }

    String* nearLabelStr = Dict_getString(msg, (&(String) { .bytes = "tar", .len = CString_strlen("tar") }));
    uint64_t label;
    if (!nearLabelStr || nearLabelStr->len != 8) {
        do { if (gprp->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(gprp->log, Log_Level_DEBUG, "GetPeersResponder.c", 54, "getPeers does not contain proper target"); } } while (0);
        return;
    } else {
        uint64_t label_be;
        Bits__memcpy(&label_be,nearLabelStr->bytes,8,"GetPeersResponder.c",58);
        label = ((uint64_t)__builtin_bswap64(((uint64_t)(label_be))));
    }

    struct Address** ptrList =
        Allocator__calloc((tmpAlloc),(sizeof(uintptr_t)),(8 +1),"GetPeersResponder.c",63);
    ptrList[8 -1] = gprp->selfAddr;
    ...
    for (int i = 0; i < gprp->peers->length; i++) {
        ...
        switch (j) {
            default: __builtin_memmove(ptrList,&ptrList[1],(j - 1) * sizeof(char*));
    ...
    do { if (!((!ptrList[8]))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("GetPeersResponder.c"), (90), "(!ptrList[8])"); } } while (0);
    struct Address* addrList =
        Allocator__calloc((tmpAlloc),(sizeof(struct Address)),(8 +1),"GetPeersResponder.c",92);
    int size = 0;
    for (int i = 0; i < 8; i++) {
        if (!ptrList[i]) { continue; }
        Bits__memcpy(&addrList[size++],ptrList[i],sizeof(struct Address),"GetPeersResponder.c",96);
    }
    do { if (!((Bits_isZero(&addrList[8], sizeof(struct Address))))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("GetPeersResponder.c"), (98), "(Bits_isZero(&addrList[8], sizeof(struct Address)))"); } } while (0);
    ...
    Dict_putString(responseDict, String_new("txid", tmpAlloc), txid, tmpAlloc);
...
struct GetPeersResponder* GetPeersResponder_new(struct Allocator* allocator,
                                                ...
                                                struct EncodingScheme* myScheme)
{
    struct Allocator* alloc = Allocator__child((allocator),"GetPeersResponder.c",122);
    struct GetPeersResponder_pvt* gprp =
        Allocator__calloc((alloc),(sizeof(struct GetPeersResponder_pvt)),(1),"GetPeersResponder.c",124);
    (gprp)->Identity_verifier = ((unsigned long) 0xd8d0ba082c9d9665ull);
    gprp->peers = peers;
    gprp->log = log;
    gprp->alloc = alloc;
