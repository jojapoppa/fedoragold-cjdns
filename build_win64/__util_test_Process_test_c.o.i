# 1 "./util/test/Process_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/test/Process_test.c"
# 15 "./util/test/Process_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x09221105426730161;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x023855772796539765;
...
struct Linker_x02906541482645628;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x07823667340961824;
...
struct Linker_x0006240307440060988;
...
struct Linker_x06989143502739212;
...
# 16 "./util/test/Process_test.c" 2
# 1 "./util/events/EventBase.h" 1
# 20 "./util/events/EventBase.h"
struct Linker_x035419977484429155;
...
# 17 "./util/test/Process_test.c" 2
# 1 "./util/events/Pipe.h" 1
# 20 "./util/events/Pipe.h"
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0272804659627931;
...
# 21 "./util/events/Pipe.h" 2


struct Linker_x06908150605201497;

# 1 "/usr/lib/gcc/x86_64-w64-mingw32/9.2-win32/include/stdbool.h" 1 3 4
# 26 "./util/events/Pipe.h" 2
...
# 73 "./util/events/Pipe.h" 3 4
                              _Bool 
# 73 "./util/events/Pipe.h"
                                   attemptToCreate,
                               struct EventBase* eb,
                               struct Except* eh,
                               struct Allocator* userAlloc);
...
struct Pipe* Pipe_forFiles(int inFd,
                           ...
                           struct Log* logger,
                           struct Allocator* userAlloc);
# 18 "./util/test/Process_test.c" 2
# 1 "./util/events/Timeout.h" 1
# 20 "./util/events/Timeout.h"
struct Linker_x01257978555909578;
...
# 22 "./util/test/Process_test.c" 2

# 1 "./util/log/FileWriterLog.h" 1
# 21 "./util/log/FileWriterLog.h"
struct Linker_x036699820640158887;
...
# 24 "./util/test/Process_test.c" 2
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x01744739472368202;
...
# 379 "/usr/share/mingw-w64/include/stdlib.h" 3
...
{
    do { if (!((!status))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (46), "(!status)"); } } while (0);
    struct Context* c = (__extension__ ({ __typeof__((struct Context*) p->userData) Identity_ptr = ((struct Context*) p->userData); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (47), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull))"); } } while (0); Identity_ptr; }));

    struct Allocator* alloc = Allocator__child((c->alloc),"Process_test.c",49);
    uint8_t* bytes = Allocator__calloc((alloc),(CString_strlen("IT WORKS!") + 1),(1),"Process_test.c",50);
    Bits__memcpy(bytes,"IT WORKS!",CString_strlen("IT WORKS!"),"Process_test.c",51);
    struct Message* m = Allocator__clone((alloc),((&(struct Message) { .length = CString_strlen("IT WORKS!"), .padding = 0, .capacity = CString_strlen("IT WORKS!"), .alloc = alloc, .bytes = bytes })),sizeof(*((&(struct Message) { .length = CString_strlen("IT WORKS!"), .padding = 0, .capacity = CString_strlen("IT WORKS!"), .alloc = alloc, .bytes = bytes }))),"Process_test.c",52)
    ...
    Allocator__free((alloc),"Process_test.c",61);
}

static __attribute__ ((warn_unused_result)) struct Iface* receiveMessageParent(struct Message* msg, struct Iface* iface)
{
    struct Context* c = (__extension__ ({ __typeof__((struct Context*) iface) Identity_ptr = ((struct Context*) iface); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (66), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull))"); } } while (0); Identity_ptr; }));
    do { if (!((msg->length == (int)CString_strlen("INDEED")))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (67), "(msg->length == (int)CString_strlen(\"INDEED\"))"); } } while (0);
    do { if (!((!__builtin_memcmp(msg->bytes,"INDEED",CString_strlen("INDEED"))))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (68), "(!__builtin_memcmp(msg->bytes,\"INDEED\",CString_strlen(\"INDEED\")))"); } } while (0);
    Allocator__free((c->alloc),"Process_test.c",69);
    return 
# 70 "./util/test/Process_test.c" 3 4
          ((void *)0)
# 70 "./util/test/Process_test.c"
              ;
}

static void timeout(void* vNULL)
{
    do { if (!((!"timed out."))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (75), "(!\"timed out.\")"); } } while (0);
}

static void onConnectionChild(struct Pipe* p, int status)
{
    do { if (!((!status))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (80), "(!status)"); } } while (0);
    printf("Child connected\n");
}

static __attribute__ ((warn_unused_result)) struct Iface* receiveMessageChild(struct Message* msg, struct Iface* iface)
{
    struct Context* c = (__extension__ ({ __typeof__((struct Context*) iface) Identity_ptr = ((struct Context*) iface); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (86), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x68d25e99595081d0ull))"); } } while (0); Identity_ptr; }));
    struct Message* m = Message_clone(msg, c->alloc);
    printf("Child received message\n");
    do { if (!((m->length == (int)CString_strlen("IT WORKS!")))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (89), "(m->length == (int)CString_strlen(\"IT WORKS!\"))"); } } while (0);
    do { if (!((!__builtin_memcmp(m->bytes,"IT WORKS!",CString_strlen("IT WORKS!"))))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (90), "(!__builtin_memcmp(m->bytes,\"IT WORKS!\",CString_strlen(\"IT WORKS!\")))"); } } while (0);

    Message_shift(m, -((int)CString_strlen("IT WORKS!")), 
# 92 "./util/test/Process_test.c" 3 4
                                                     ((void *)0)
# 92 "./util/test/Process_test.c"
                                                         );
    Message_push(m, "INDEED", CString_strlen("INDEED"), 
# 93 "./util/test/Process_test.c" 3 4
                                                       ((void *)0)
# 93 "./util/test/Process_test.c"
                                                           );

    Iface_send(&c->iface, m);


    Allocator__free((c->alloc),"Process_test.c",98);

    return 
# 100 "./util/test/Process_test.c" 3 4
          ((void *)0)
# 100 "./util/test/Process_test.c"
              ;
}

static void child(char* name, struct Context* ctx)
{
    struct Pipe* pipe = Pipe_named("/tmp", name, ctx->base, 
# 105 "./util/test/Process_test.c" 3 4
                                                              ((void *)0)
# 105 "./util/test/Process_test.c"
                                                                  , ctx->alloc);
    pipe->logger = ctx->log;
    ...
    Timeout__setTimeout((timeout), (
# 111 "./util/test/Process_test.c" 3 4
   ((void *)0)
# 111 "./util/test/Process_test.c"
   ), (2000), (ctx->base), (ctx->alloc), "Process_test.c", 111);
    EventBase_beginLoop(ctx->base);
}

int Process_test_main(int argc, char** argv);int Process_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((1<<20),"Process_test.c",117);
    struct EventBase* eb = EventBase_new(alloc);
    struct Log* log = FileWriterLog_new(
# 119 "./util/test/Process_test.c" 3
                                       (__acrt_iob_func(1))
# 119 "./util/test/Process_test.c"
                                             , alloc);
    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"Process_test.c",120);
    (ctx)->Identity_verifier = ((unsigned long) 0x68d25e99595081d0ull);
    ctx->alloc = alloc;
    ctx->base = eb;
    ctx->log = log;
    ...
    struct Random* rand = Random_new(alloc, log, 
# 132 "./util/test/Process_test.c" 3 4
                                                ((void *)0)
# 132 "./util/test/Process_test.c"
                                                    );
    char name[32] = {0};
    Random_base32(rand, (uint8_t*)name, 31);

    struct Pipe* pipe = Pipe_named("/tmp", name, eb, 
# 136 "./util/test/Process_test.c" 3 4
                                                       ((void *)0)
# 136 "./util/test/Process_test.c"
                                                           , alloc);
    pipe->logger = log;
    pipe->userData = ctx;
    ...
    do { if (!((path != 
# 144 "./util/test/Process_test.c" 3 4
   ((void *)0)
# 144 "./util/test/Process_test.c"
   ))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (144), "(path != ((void *)0))"); } } while (0);
# 153 "./util/test/Process_test.c"
        do { if (!((path[0] == '/'))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (153), "(path[0] == '/')"); } } while (0);


    char* args[] = { "Process_test", "child", name, 
# 156 "./util/test/Process_test.c" 3 4
                                                   ((void *)0) 
# 156 "./util/test/Process_test.c"
                                                        };

    do { if (!((!Process_spawn(path, args, eb, alloc, 
# 158 "./util/test/Process_test.c" 3 4
   ((void *)0)
# 158 "./util/test/Process_test.c"
   )))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Process_test.c"), (158), "(!Process_spawn(path, args, eb, alloc, ((void *)0)))"); } } while (0);

    Timeout__setTimeout((timeout), (
# 160 "./util/test/Process_test.c" 3 4
   ((void *)0)
# 160 "./util/test/Process_test.c"
   ), (2000), (eb), (alloc), "Process_test.c", 160);
