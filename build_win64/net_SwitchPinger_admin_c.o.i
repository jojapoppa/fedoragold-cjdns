# 1 "net/SwitchPinger_admin.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "net/SwitchPinger_admin.c"
# 15 "net/SwitchPinger_admin.c"
# 1 "./admin/Admin.h" 1
# 18 "./admin/Admin.h"
...
struct Linker_x044654483578263626;
...
# 1 "./util/Assert.h" 1
# 20 "./util/Assert.h"
struct Linker_x06063844086305579;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
__attribute__((__noreturn__))
...
struct Linker_x01116553482837821;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x05147069697250959;
...
struct Linker_x024520172673711405;
...
struct Linker_x04161952527707702;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x08151200290359573;
...
struct Linker_x003937299192658528;
...
struct Linker_x013130104774690987;
...
struct Admin* Admin_new(struct AddrIface* ai,
                        ...
                        String* password);
# 16 "net/SwitchPinger_admin.c" 2
# 1 "./benc/String.h" 1
# 20 "./benc/String.h"
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x06400279385049401;
...
struct Linker_x06583649055043521;
...
# 17 "net/SwitchPinger_admin.c" 2

# 1 "./benc/Int.h" 1
# 19 "net/SwitchPinger_admin.c" 2
# 1 "./dht/Address.h" 1
# 21 "./dht/Address.h"
struct Linker_x09440699841229772;
# 34 "./dht/Address.h"
struct Address
{
...
struct Linker_x06249043031868098;
...
struct Linker_x047572642904163054;
...
struct SwitchPinger* SwitchPinger_new(struct EventBase* eventBase,
                                      ...
                                      struct Allocator* alloc);
# 21 "net/SwitchPinger_admin.c" 2
# 1 "./net/SwitchPinger_admin.h" 1
# 18 "./net/SwitchPinger_admin.h"
# 1 "./admin/Admin.h" 1
# 19 "./net/SwitchPinger_admin.h" 2
...
struct Linker_x008120102532606444;

void SwitchPinger_admin_register(struct SwitchPinger* sp,
                                 struct Admin* admin,
                                 struct Allocator* alloc);
# 22 "net/SwitchPinger_admin.c" 2

# 1 "./util/AddrTools.h" 1
# 19 "./util/AddrTools.h"
struct Linker_x06601386152238684;
...
# 24 "net/SwitchPinger_admin.c" 2
# 1 "./crypto/Key.h" 1
# 22 "./crypto/Key.h"
struct Linker_x08452935513490154;
...
static void adminPingOnResponse(struct SwitchPinger_Response* resp, void* vping)
{
    ...
    if (resp->res == SwitchPinger_Result_LABEL_MISMATCH) {
        ...
        Dict_putString(rd, String_new("rpath", pingAlloc), pathStr, pingAlloc);
    }

    Dict_putInt(rd, String_new("version", pingAlloc), resp->version, pingAlloc);
    Dict_putInt(rd, String_new("ms", pingAlloc), resp->milliseconds, pingAlloc);
    Dict_putString(rd, String_new("result", pingAlloc), SwitchPinger_resultString(resp->res), pingAlloc);
    Dict_putString(rd, String_new("path", pingAlloc), ping->path, pingAlloc);
    if (resp->data) {
        Dict_putString(rd, String_new("data", pingAlloc), resp->data, pingAlloc);
    }

    if (!Bits_isZero(resp->key, 32)) {
        Dict_putString(rd, String_new("key", pingAlloc), Key_stringify(resp->key, pingAlloc), pingAlloc);
    }
...
static void adminPing(Dict* args, void* vcontext, String* txid, struct Allocator* requestAlloc)
{
    ...
    String* data = Dict_getString(args, (&(String) { .bytes = "data", .len = CString_strlen("data") }));
    int64_t* keyPing = Dict_getInt(args, (&(String) { .bytes = "keyPing", .len = CString_strlen("keyPing") }));
    uint32_t timeout = (timeoutPtr) ? *timeoutPtr : 2000;
    uint64_t path;
    String* err = 
# 80 "net/SwitchPinger_admin.c" 3 4
                 ((void *)0)
# 80 "net/SwitchPinger_admin.c"
                     ;
    if (pathStr->len != 19 || AddrTools_parsePath(&path, (uint8_t*) pathStr->bytes)) {
        err = (&(String) { .bytes = "path was not parsable.", .len = CString_strlen("path was not parsable.") });
    } else {
        ...
        if (!ping) {
            err = (&(String) { .bytes = "no open slots to store ping, try later.", .len = CString_strlen("no open slots to store ping, try later.") });
        } else {
            ping->onResponseContext = Allocator__clone((ping->pingAlloc),((&(struct Ping) { .context = context, .txid = ((txid) ? String_newBinary(txid->bytes, txid->len, ping->pingAlloc) : 
# 94 "net/SwitchPinger_admin.c" 3 4
                                     ((void *)0)
# 94 "net/SwitchPinger_admin.c"
                                     ), .path = ((pathStr) ? String_newBinary(pathStr->bytes, pathStr->len, ping->pingAlloc) : 
# 94 "net/SwitchPinger_admin.c" 3 4
                                     ((void *)0)
# 94 "net/SwitchPinger_admin.c"
                                     ) })),sizeof(*((&(struct Ping) { .context = context, .txid = ((txid) ? String_newBinary(txid->bytes, txid->len, ping->pingAlloc) : 
# 94 "net/SwitchPinger_admin.c" 3 4
                                     ((void *)0)
# 94 "net/SwitchPinger_admin.c"
                                     ), .path = ((pathStr) ? String_newBinary(pathStr->bytes, pathStr->len, ping->pingAlloc) : 
...
# 94 "net/SwitchPinger_admin.c"
    ...
    if (err) {
        Dict d = (&(struct Dict_Entry) { .key = (&(String) { .bytes = "error", .len = CString_strlen("error") }), .val = (&(Object) { .type = Object_STRING, .as.string = err }), .next = 
# 103 "net/SwitchPinger_admin.c" 3 4
                ((void *)0) 
# 103 "net/SwitchPinger_admin.c"
                });
...
void SwitchPinger_admin_register(struct SwitchPinger* sp,
                                 ...
                                 struct Allocator* allocator)
{
    struct Allocator* alloc = Allocator__child((allocator),"SwitchPinger_admin.c",112);
    struct Context* ctx = Allocator__clone((alloc),((&(struct Context) { .switchPinger = sp, .alloc = alloc, .admin = admin })),sizeof(*((&(struct Context) { .switchPinger = sp, .alloc = alloc, .admin = admin }))),"SwitchPinger_admin.c",113)



       ;

    Admin_registerFunctionWithArgCount( "SwitchPinger_ping", adminPing, ctx, 
# 119 "net/SwitchPinger_admin.c" 3 4
   1
# 119 "net/SwitchPinger_admin.c"
   , ((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, { .name = "data", .required = 0, .type = "String" }, { .name = "keyPing", .required = 0, .type = "Int" } }), (sizeof(((struct Admin_FunctionArg[]) { { .name = "path", .required = 1, .type = "String" }, { .name = "timeout", .required = 0, .type = "Int" }, { .name = "data", .required = 0, .type = "String" }, { .name = "keyPing", .required = 0, .type = "Int" } })) / sizeof(struct Admin_FunctionArg)), admin)
