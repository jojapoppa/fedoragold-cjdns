# 1 "./util/platform/test/Sockaddr_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/platform/test/Sockaddr_test.c"
# 15 "./util/platform/test/Sockaddr_test.c"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x06944673538356911;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x09996545245024715;
...
struct Linker_x08461393201363858;
...
# 17 "./util/platform/test/Sockaddr_test.c" 2

# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x041720981577229743;
...
# 19 "./util/platform/test/Sockaddr_test.c" 2

static void expectFailure(char* address)
{
    struct Sockaddr_storage ss;
    do { if (!((Sockaddr_parse(address, &ss)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (23), "(Sockaddr_parse(address, &ss))"); } } while (0);
}

static void expectConvert(char* address, char* expectedOutput)
{
    struct Sockaddr_storage ss;
    do { if (!((!Sockaddr_parse(address, &ss)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (29), "(!Sockaddr_parse(address, &ss))"); } } while (0);
    struct Allocator* alloc = MallocAllocator__new((20000),"Sockaddr_test.c",30);
    char* outAddr = Sockaddr_print(&ss.addr, alloc);
    do { if (!((outAddr))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (32), "(outAddr)"); } } while (0);
    do { if (!((CString_strlen(outAddr) == CString_strlen(expectedOutput)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (33), "(CString_strlen(outAddr) == CString_strlen(expectedOutput))"); } } while (0);
    do { if (!((!CString_strcmp(outAddr, expectedOutput)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (34), "(!CString_strcmp(outAddr, expectedOutput))"); } } while (0);
    Allocator__free((alloc),"Sockaddr_test.c",35);
}
...
static void parse()
{
    ...
    do { if (!((Sockaddr_asNative(&test.addr) == ((uint8_t*)&test) + 8))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Sockaddr_test.c"), (46), "(Sockaddr_asNative(&test.addr) == ((uint8_t*)&test) + 8)"); } } while (0);
...
int Sockaddr_test_main(int argc, char** argv);int Sockaddr_test_main(int argc, char** argv)
{
    parse();
    return 0;
}
