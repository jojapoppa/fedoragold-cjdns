...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x06666656275544818;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x0060315784507299686;
...
struct Linker_x0642243475386971;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x010068016863621487;
...
struct Linker_x08634961143648692;
...
struct Linker_x011083727052720915;
...
struct Linker_x008856128425473297;
...
struct Linker_x008288158259832845;
                                  ...
                                  struct Except* eh)
{
    ...
    *addrFam = Sockaddr_getFamily(sa);
    if (*addrFam != Sockaddr_AF_INET && *addrFam != Sockaddr_AF_INET6) {
        Except__throw("NetDev.c", 33, eh, "Unknown address type for address [%s]", *printedAddr);
    }
    ...
    if (sa->prefix > prefixMax) {
        Except__throw("NetDev.c", 40, eh, "prefix [%u] must be less than %d", sa->prefix, prefixMax);
    }

    int len = Sockaddr_getAddress(sa, addr);
    if (len < 0 || len != prefixMax / 8) {
        Except__throw("NetDev.c", 45, eh, "Invalid sockaddr [%s]", *printedAddr);
    }
}
                       ...
                       struct Except* eh)
{
    ...
    void* addr;
    struct Allocator* alloc;
    uint8_t UniqueName_6b3bcbd7c1d6fe772c3a[4096]; alloc = BufferAllocator__new((UniqueName_6b3bcbd7c1d6fe772c3a),(4096),"NetDev.c",58);;

    checkAddressAndPrefix(sa, &addrFam, &printedAddr, &addr, alloc, eh);

    do { if (logger && Log_Level_INFO >= Log_Level_DEBUG) { Log_print(logger, Log_Level_INFO, "NetDev.c", 62, "Setting IP address [%s] on interface [%s]", printedAddr, ifName); } } while (0)
                                 ;
...
# 103 "util/platform/netdev/NetDev.c"
void NetDev_setRoutes(const char* ifName,
                      struct Sockaddr** prefixSet,
                      int prefixCount,
                      struct Log* logger,
