# 1 "dht/dhtcore/SearchStore.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "dht/dhtcore/SearchStore.c"
# 15 "dht/dhtcore/SearchStore.c"
# 1 "./dht/Address.h" 1
# 18 "./dht/Address.h"
# 1 "./benc/String.h" 1
# 18 "./benc/String.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x023116397786826637;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x021143384566007084;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x04552484120574527;
...
struct Linker_x044072423377341674;
...
struct Linker_x05776913949548579;
# 34 "./dht/Address.h"
struct Address
{
...
# 16 "dht/dhtcore/SearchStore.c" 2
# 1 "./dht/dhtcore/SearchStore.h" 1
# 25 "./dht/dhtcore/SearchStore.h"
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x0024878634336894967;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 26 "./dht/dhtcore/SearchStore.h" 2

struct Linker_x0030107663717191535;
...
# 17 "dht/dhtcore/SearchStore.c" 2
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x01393090328266584;
...
# 18 "dht/dhtcore/SearchStore.c" 2

# 1 "./util/AverageRoller.h" 1
# 21 "./util/AverageRoller.h"
# 1 "./util/events/EventBase.h" 1
# 20 "./util/events/EventBase.h"
struct Linker_x07585016656949282;
...
struct Linker_x041879936319014166;
...
# 20 "dht/dhtcore/SearchStore.c" 2
# 1 "./util/Endian.h" 1
# 21 "dht/dhtcore/SearchStore.c" 2
# 1 "./util/events/Time.h" 1
# 20 "./util/events/Time.h"
struct Linker_x07093961303119152;
...
# 22 "dht/dhtcore/SearchStore.c" 2
...
struct SearchStore_Search_pvt
{
    ...
    struct Address searchStack[256];


    struct SearchStore_Node nodesAsked[256];
};
...
struct SearchStore* SearchStore_new(struct Allocator* allocator, struct Log* logger)
{
    return Allocator__clone((allocator),((&(struct SearchStore) { .allocator = allocator, .logger = logger })),sizeof(*((&(struct SearchStore) { .allocator = allocator, .logger = logger }))),"SearchStore.c",51)
                                                 ...
                                                 struct Allocator* alloc)
{
    struct SearchStore_Search_pvt* search =
        Allocator__clone((alloc),((&(struct SearchStore_Search_pvt) { .pub = { .callbackContext = 
# 63 "dht/dhtcore/SearchStore.c" 3 4
       ((void *)0)
# 63 "dht/dhtcore/SearchStore.c"
       , .store = store, .alloc = alloc } })),sizeof(*((&(struct SearchStore_Search_pvt) { .pub = { .callbackContext = 
# 63 "dht/dhtcore/SearchStore.c" 3 4
       ((void *)0)
# 63 "dht/dhtcore/SearchStore.c"
       , .store = store, .alloc = alloc } }))),"SearchStore.c",63)
           ...
           ;
    Bits__memcpy(search->searchTarget,searchTarget,16,"SearchStore.c",70);
...
int SearchStore_addNodeToSearch(struct Address* addr, struct SearchStore_Search* search)
{
    ...
    for (int i = 0; i < 256; i++) {
        if (!__builtin_memcmp(addr->key,pvtSearch->nodesAsked[i].address.key,32)) {
    ...
    Bits__memcpy(&pvtSearch->searchStack[index],addr,(8 + 16 + 32 + 8),"SearchStore.c",99);
...
struct SearchStore_Node* SearchStore_getNextNode(struct SearchStore_Search* search)
{
    ...
    if (!pvtSearch->searchStackSize) {
        return 
# 109 "dht/dhtcore/SearchStore.c" 3 4
              ((void *)0)
# 109 "dht/dhtcore/SearchStore.c"
                  ;
    }

    struct SearchStore_Node* nn = &pvtSearch->nodesAsked[pvtSearch->nodesAskedIndex];
    Bits__memcpy(&nn->address,&pvtSearch->searchStack[--pvtSearch->searchStackSize],sizeof(struct Address),"SearchStore.c",113)

                                            ;
    nn->search = search;

    pvtSearch->nodesAskedIndex = (pvtSearch->nodesAskedIndex + 1) % 256;
    return nn;
}
