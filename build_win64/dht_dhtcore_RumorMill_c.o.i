# 1 "dht/dhtcore/RumorMill.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "dht/dhtcore/RumorMill.c"
# 15 "dht/dhtcore/RumorMill.c"
# 1 "./dht/dhtcore/RumorMill.h" 1
# 22 "./dht/dhtcore/RumorMill.h"
# 1 "./dht/Address.h" 1
# 18 "./dht/Address.h"
# 1 "./benc/String.h" 1
# 18 "./benc/String.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x031991522935774386;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x07540941728630912;
...
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x09985741306865592;
...
struct Linker_x0568185286606161;
...
struct Linker_x010402803979422615;
# 34 "./dht/Address.h"
struct Address
{
...
# 23 "./dht/dhtcore/RumorMill.h" 2

# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06492968881605772;
...
struct Linker_x01992130985850249;
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x05522717262921519;
...
# 21 "dht/dhtcore/RumorMill.c" 2
# 1 "./util/Endian.h" 1
# 22 "dht/dhtcore/RumorMill.c" 2
# 1 "./util/Defined.h" 1
# 23 "dht/dhtcore/RumorMill.c" 2
# 31 "dht/dhtcore/RumorMill.c"
...
{
    uint64_t xor = ((uint64_t)__builtin_bswap64(((uint64_t)(badAddr->ip6.longs.one_be ^ selfAddr->ip6.longs.one_be))));
    return Bits_log2x64(xor) + Bits_log2x64(badAddr->path);
}

static struct Address* getWorst(struct RumorMill_pvt* rm)
{
    struct Address* worst = 
# 52 "dht/dhtcore/RumorMill.c" 3 4
                           ((void *)0)
# 52 "dht/dhtcore/RumorMill.c"
                               ;
    ...
    do { if (!((worst))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RumorMill.c"), (61), "(worst)"); } } while (0);
    return worst;
}

static struct Address* getBest(struct RumorMill_pvt* rm)
{
    if (rm->pub.count == 0) { return 
# 67 "dht/dhtcore/RumorMill.c" 3 4
                                    ((void *)0)
# 67 "dht/dhtcore/RumorMill.c"
                                        ; }
    struct Address* best = 
# 68 "dht/dhtcore/RumorMill.c" 3 4
                          ((void *)0)
# 68 "dht/dhtcore/RumorMill.c"
                              ;
    ...
    do { if (!((best))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RumorMill.c"), (77), "(best)"); } } while (0);
    return best;
}

void RumorMill__addNode(struct RumorMill* mill, struct Address* addr, const char* file, int line)
{
    struct RumorMill_pvt* rm = (__extension__ ({ __typeof__((struct RumorMill_pvt*) mill) Identity_ptr = ((struct RumorMill_pvt*) mill); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x14869bb56f971216ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RumorMill.c"), (83), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x14869bb56f971216ull))"); } } while (0); Identity_ptr; }));

    Address_getPrefix(addr);
    do { if (!((addr->protocolVersion))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RumorMill.c"), (86), "(addr->protocolVersion)"); } } while (0);

    for (int i = 0; i < rm->pub.count; i++) {
        if (rm->pub.addresses[i].path == addr->path &&
            !__builtin_memcmp(rm->pub.addresses[i].key,addr->key,32))
        {
            return;
        }
    }

    if (!__builtin_memcmp(addr->key,rm->selfAddr->key,32)) { return; }
    ...
    Bits__memcpy(replace,addr,sizeof(struct Address),"RumorMill.c",104);

    if (1) {
        uint8_t addrStr[60];
        Address_print(addrStr, addr);
        do { if (rm->log && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(rm->log, Log_Level_DEBUG, "RumorMill.c", 109, "[%s] addNode(%s) count[%d] from [%s:%d]", rm->pub.name, addrStr, rm->pub.count, file, line); } } while (0)
                                                                            ;
    }
}


# 114 "dht/dhtcore/RumorMill.c" 3 4
_Bool 
# 114 "dht/dhtcore/RumorMill.c"
    RumorMill_getNode(struct RumorMill* mill, struct Address* output)
{
    struct RumorMill_pvt* rm = (__extension__ ({ __typeof__((struct RumorMill_pvt*) mill) Identity_ptr = ((struct RumorMill_pvt*) mill); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x14869bb56f971216ull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("RumorMill.c"), (116), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x14869bb56f971216ull))"); } } while (0); Identity_ptr; }));
    if (!rm->pub.count) { return 
# 117 "dht/dhtcore/RumorMill.c" 3 4
                                0
# 117 "dht/dhtcore/RumorMill.c"
                                     ; }
    struct Address* best = getBest(rm);
    if (output) {
        Bits__memcpy(output,best,sizeof(struct Address),"RumorMill.c",120);
    }

    rm->pub.count--;
    if (&rm->pub.addresses[rm->pub.count] != best) {
        Bits__memcpy(best,&rm->pub.addresses[rm->pub.count],sizeof(struct Address),"RumorMill.c",125);
    }
    return 
# 127 "dht/dhtcore/RumorMill.c" 3 4
          1
# 127 "dht/dhtcore/RumorMill.c"
              ;
}
...
struct RumorMill* RumorMill_new(struct Allocator* allocator,
                                ...
                                const char* name)
{
    struct Allocator* alloc = Allocator__child((allocator),"RumorMill.c",136);
    Address_getPrefix(selfAddr);

    struct RumorMill_pvt* rm = Allocator__calloc((alloc),(sizeof(struct RumorMill_pvt)),(1),"RumorMill.c",139);
    rm->pub.addresses = Allocator__calloc((alloc),(sizeof(struct Address)),(capacity),"RumorMill.c",140);
    rm->capacity = capacity;
    rm->selfAddr = Allocator__clone((alloc),(selfAddr),sizeof(*(selfAddr)),"RumorMill.c",142);
    rm->log = log;
    rm->pub.name = name;
    (rm)->Identity_verifier = ((unsigned long) 0x14869bb56f971216ull);
