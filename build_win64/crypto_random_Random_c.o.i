# 1 "crypto/random/Random.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "crypto/random/Random.c"
# 15 "crypto/random/Random.c"
...
# 19 "./util/crypto.h"
struct Linker_x022875850229552142;
# 30 "./util/crypto.h"
...
struct Linker_x004236207165141126;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x009955817690507618;
...
struct Linker_x006019723356335538;
...
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x015036978624149056;
...
struct Linker_x00012499125889462448;
...
struct Linker_x02869126270342013;
...
# 22 "crypto/random/Random.c" 2

# 1 "./crypto/random/seed/SystemRandomSeed.h" 1
# 19 "./crypto/random/seed/SystemRandomSeed.h"
# 1 "./crypto/random/seed/SystemRandomSeed.h" 1
# 20 "./crypto/random/seed/SystemRandomSeed.h" 2

struct Linker_x05971727744435393;
...
struct RandomSeed* SystemRandomSeed_new(RandomSeed_Provider* additionalProviders,
                                        ...
                                        struct Log* logger,
                                        struct Allocator* alloc);
# 24 "crypto/random/Random.c" 2

# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x07399324816003126;
...
# 28 "crypto/random/Random.c" 2

# 1 "./util/Endian.h" 1
# 30 "crypto/random/Random.c" 2
# 104 "crypto/random/Random.c"
...
# 159 "crypto/random/Random.c"
void Random_addRandom(struct Random* rand, uint32_t randomNumber)
{
    (__extension__ ({ __typeof__(rand) Identity_ptr = (rand); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random.c"), (161), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull))"); } } while (0); Identity_ptr; }));

    rand->seedGen->elements.collectedEntropy[rand->addRandomCounter % 8] ^=
        (((randomNumber) << (rand->addRandomCounter / 8)) | ((randomNumber) >> (31 - (rand->addRandomCounter / 8))));
...
{
    uint64_t nonce = (rand->nonce);
    crypto_stream_salsa20_tweet_xor((uint8_t*)rand->buff,
                              (uint8_t*)rand->buff,
                              128,
                              (uint8_t*)&nonce,
...
static uintptr_t randomCopy(struct Random* rand, uint8_t* location, uint64_t count)
{
    ...
    if (num > (uintptr_t)(128 - rand->nextByte)) {
        num = (128 - rand->nextByte);
    }
    Bits__memcpy(location,&rand->buff[rand->nextByte],num,"Random.c",191);
    rand->nextByte += num;
    return num;
}
...
void Random_bytes(struct Random* rand, uint8_t* location, uint64_t count)
{
    (__extension__ ({ __typeof__(rand) Identity_ptr = (rand); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random.c"), (198), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull))"); } } while (0); Identity_ptr; }));
    if (count > 128) {

        crypto_stream_salsa20_tweet_xor((uint8_t*)location,
                                  (uint8_t*)location,
                                  count,
                                  (uint8_t*)&rand->nonce,
                                  (uint8_t*)rand->tempSeed);
...
void Random_base32(struct Random* rand, uint8_t* output, uint32_t length)
{
    (__extension__ ({ __typeof__(rand) Identity_ptr = (rand); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Random.c"), (223), "(Identity_ptr->Identity_verifier == ((unsigned long) 0xb2bcbdb81fa7e8cbull))"); } } while (0); Identity_ptr; }));
    uint64_t index = 0;
    for (;;) {
        uint8_t bin[16];
        Random_bytes(rand, bin, 16);
        int ret = Base32_encode(&output[index], length - index, (uint8_t*)bin, 16);
        if (ret == -2 || index + ret == length) {
            break;
        }
        index += ret;
    }
...
struct Random* Random_newWithSeed(struct Allocator* alloc,
                                  ...
                                  struct Except* eh)
{
    union Random_SeedGen* seedGen = Allocator__calloc((alloc),(sizeof(union Random_SeedGen)),(1),"Random.c",242);

    if (RandomSeed_get(seed, seedGen->buff)) {
        Except__throw("Random.c", 245, eh, "Unable to initialize secure random number generator");
    }

    struct Random* rand = Allocator__calloc((alloc),(sizeof(struct Random)),(1),"Random.c",248);
    rand->seedGen = seedGen;
    rand->seed = seed;
    rand->nextByte = 128;

    (rand)->Identity_verifier = ((unsigned long) 0xb2bcbdb81fa7e8cbull);
...
struct Random* Random_new(struct Allocator* alloc, struct Log* logger, struct Except* eh)
{
    struct RandomSeed* rs = SystemRandomSeed_new(
# 264 "crypto/random/Random.c" 3 4
                                                ((void *)0)
# 264 "crypto/random/Random.c"
                                                    , 0, logger, alloc);
