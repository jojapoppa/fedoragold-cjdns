# 1 "./util/test/Seccomp_test.c"
# 1 "/home/jojapoppa/fedoragold-cjdns//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./util/test/Seccomp_test.c"
# 15 "./util/test/Seccomp_test.c"
# 1 "./util/log/FileWriterLog.h" 1
# 18 "./util/log/FileWriterLog.h"
...
# 1 "./util/Linker.h" 1
# 20 "./util/Assert.h" 2
struct Linker_x017105181111627754;
# 33 "./util/Assert.h"
__attribute__((__format__ (__printf__, 1, 2)))
...
struct Linker_x05955928205800813;
...
struct Allocator* Allocator_new(unsigned long sizeLimit,
                                ...
                                const char* fileName,
                                int lineNum);
# 19 "./util/log/FileWriterLog.h" 2
# 1 "./util/log/Log.h" 1
# 20 "./util/log/Log.h"
struct Linker_x06573181418786076;
...
void Log_print(struct Log* log,
               ...
               const char* format,
               ...);
# 20 "./util/log/FileWriterLog.h" 2

struct Linker_x06914565115907003;
...
# 19 "./util/test/Seccomp_test.c" 2
# 1 "./util/events/EventBase.h" 1
# 20 "./util/events/EventBase.h"
struct Linker_x06746045311081184;
...
# 20 "./util/test/Seccomp_test.c" 2
# 1 "./util/events/Process.h" 1
# 20 "./util/events/Process.h"
struct Linker_x036477888573561734;
...
# 23 "./util/events/Process.h" 2


# 24 "./util/events/Process.h"
...
# 1 "./util/Bits.h" 1
# 21 "./util/Bits.h"
struct Linker_x0300860840269781;
...
struct Linker_x06928394625139738;
...
struct Pipe* Pipe_forFiles(int inFd,
                           ...
                           struct Log* logger,
                           struct Allocator* userAlloc);
# 22 "./util/test/Seccomp_test.c" 2
# 1 "./util/events/Timeout.h" 1
# 20 "./util/events/Timeout.h"
struct Linker_x08155849103560204;
...
# 23 "./util/test/Seccomp_test.c" 2
# 1 "./util/CString.h" 1
# 20 "./util/CString.h"
struct Linker_x044571761312407165;
...
struct Linker_x09535790276044263;
...
static void onConnectionChild(struct Pipe* p, int status)
{
    ...
    Seccomp_dropPermissions(alloc, logger, 
# 45 "./util/test/Seccomp_test.c" 3 4
                                          ((void *)0)
# 45 "./util/test/Seccomp_test.c"
                                              );
    do { if (!((Seccomp_isWorking()))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (46), "(Seccomp_isWorking())"); } } while (0);

    struct Message* ok = Message_new(0, 512, alloc);
    Message_push(ok, "OK", 3, 
# 49 "./util/test/Seccomp_test.c" 3 4
                             ((void *)0)
# 49 "./util/test/Seccomp_test.c"
                                 );

    struct Iface iface = { .send = 
# 51 "./util/test/Seccomp_test.c" 3 4
                                  ((void *)0) 
# 51 "./util/test/Seccomp_test.c"
                                       };
    ...
    Timeout__setInterval((childComplete), (p->base), (10), (p->base), (alloc), "Seccomp_test.c", 57);
}

static void timeout(void* vNULL)
{
    do { if (!((!"timed out"))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (62), "(!\"timed out\")"); } } while (0);
}

static int child(char* pipeName, struct Allocator* alloc, struct Log* logger)
{
    struct EventBase* eb = EventBase_new(alloc);
    struct Pipe* pipe = Pipe_named("/tmp", pipeName, eb, 
# 68 "./util/test/Seccomp_test.c" 3 4
                                                           ((void *)0)
# 68 "./util/test/Seccomp_test.c"
                                                               , alloc);
    pipe->onConnection = onConnectionChild;
    pipe->logger = logger;
    pipe->userData = alloc;

    Timeout__setTimeout((timeout), (eb), (2000), (eb), (alloc), "Seccomp_test.c", 73);
...
{
    struct Context* ctx = (__extension__ ({ __typeof__((struct Context*) iface) Identity_ptr = ((struct Context*) iface); do { if (!((Identity_ptr->Identity_verifier == ((unsigned long) 0x1263e36e360d3d8bull)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (81), "(Identity_ptr->Identity_verifier == ((unsigned long) 0x1263e36e360d3d8bull))"); } } while (0); Identity_ptr; }));
    do { if (!((msg->length == 3))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (82), "(msg->length == 3)"); } } while (0);
    do { if (!((!__builtin_memcmp(msg->bytes,"OK",3)))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (83), "(!__builtin_memcmp(msg->bytes,\"OK\",3))"); } } while (0);
    EventBase_endLoop(ctx->eventBase);
    return 0;
}

int Seccomp_test_main(int argc, char** argv);int Seccomp_test_main(int argc, char** argv)
{
    struct Allocator* alloc = MallocAllocator__new((20000),"Seccomp_test.c",90);
    struct Log* logger = FileWriterLog_new(
# 91 "./util/test/Seccomp_test.c" 3
                                          (__acrt_iob_func(1))
# 91 "./util/test/Seccomp_test.c"
                                                , alloc);

    if (!Seccomp_exists()) {
        do { if (logger && Log_Level_DEBUG >= Log_Level_DEBUG) { Log_print(logger, Log_Level_DEBUG, "Seccomp_test.c", 94, "Seccomp not supported on this system"); } } while (0);
        return 0;
    }
    if (argc > 3 && !CString_strcmp("Seccomp_test", argv[1]) && !CString_strcmp("child", argv[2])) {
        child(argv[3], alloc, logger);
        Allocator__free((alloc),"Seccomp_test.c",99);
        return 0;
    }

    struct EventBase* eb = EventBase_new(alloc);
    struct Random* rand = Random_new(alloc, logger, 
# 104 "./util/test/Seccomp_test.c" 3 4
                                                   ((void *)0)
# 104 "./util/test/Seccomp_test.c"
                                                       );
    char name[32] = {0};
    Random_base32(rand, (uint8_t*)name, 31);

    struct Context* ctx = Allocator__calloc((alloc),(sizeof(struct Context)),(1),"Seccomp_test.c",108);
    (ctx)->Identity_verifier = ((unsigned long) 0x1263e36e360d3d8bull);
    ctx->alloc = alloc;
    ctx->iface.send = receiveMessageParent;
    ctx->eventBase = eb;

    struct Pipe* pipe = Pipe_named("/tmp", name, eb, 
# 114 "./util/test/Seccomp_test.c" 3 4
                                                       ((void *)0)
# 114 "./util/test/Seccomp_test.c"
                                                           , alloc);
    pipe->logger = logger;
    Iface_plumb(&ctx->iface, &pipe->iface);

    char* path = Process_getPath(alloc);
    char* args[] = { "Seccomp_test", "child", name, 
# 119 "./util/test/Seccomp_test.c" 3 4
                                                   ((void *)0) 
# 119 "./util/test/Seccomp_test.c"
                                                        };

    do { if (!((!Process_spawn(path, args, eb, alloc, 
# 121 "./util/test/Seccomp_test.c" 3 4
   ((void *)0)
# 121 "./util/test/Seccomp_test.c"
   )))) { Assert_failure("Assertion failure [%s:%d] [%s]\n", ("Seccomp_test.c"), (121), "(!Process_spawn(path, args, eb, alloc, ((void *)0)))"); } } while (0);

    Timeout__setTimeout((timeout), (
# 123 "./util/test/Seccomp_test.c" 3 4
   ((void *)0)
# 123 "./util/test/Seccomp_test.c"
   ), (2000), (eb), (alloc), "Seccomp_test.c", 123);
